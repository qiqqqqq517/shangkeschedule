...
- **今日**：今日课程时间轴 + 下节课卡（今日课程结束后可配置：自动显示下一次 / 已结束变淡 / 消失）+ 今日待办 + 明日预览。
- **课表**：周一至周日网格排布，左侧为节次与时间段，课程以彩色色块呈现；支持左右滑动切换周次、点击顶部周次快速跳转、长按课程块调整位置或高度。
- **日程**：日期轴 + 整月日历 + 当日时间轴，待办 / 活动 / 考试 / 作业等日程与课程同屏展示；勾选完成状态与今日页实时同步。
- **我的**：设置主入口（见下节）。
...
> 说明：本 README 暂未附带界面截图，实际界面以最新版为准。
...
- **独立作息**：可单独设置 TA 课表的节次时间（含多作息方案）；叠加视图下双方课程分别按各自作息换算位置，作息不同时课程卡显示起止时间（可在设置中开关）。
...
- 多学期并排管理，**点击任意学期卡即可把它设为当前学期**（列表实时重排，「查看」才跳转课表）。
