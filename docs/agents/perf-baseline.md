...
## PF1 实验结论（2026-09-26）：跳过「每帧重录」无收益，建议暂缓

清单 PF1 的核心推断是「背板在每帧 draw 内重录整页一层 ⇒ 静止态即 ≥2 次全屏模糊
+ 2 次整页层采样」，并据此提出「背板录制加脏标记（内容不变不重录）」。

为验证这条路的天花板，做了**量化实验**（在隔离 worktree 中进行，**未进主仓**）：
把 `GlassBackdropSourceNode.draw()` 改成**只录一次**（`recordedOnce`，
故意保留残影以测出上限），同设备同负载对比：

| 场景 | 对照组（现状：每帧重录） | 实验组（只录一次） |
|---|---|---|
| 默认页 | 2.27% · p50 6ms · p90 9ms · p95 10ms | **2.28%** · p50 6ms · p90 10ms · p95 12ms |
| 课表页 | 2.70% / 2.61% | **3.22%**（反而更差，属噪声） |

**结论：零收益。** ⇒ 「把内容录进层」这一步**不是瓶颈**，真正的大头是**模糊本身**
（blur 的 GPU 填充率）。清单那条推断在 v3.43.0 时代或许成立，
但在 v3.69.4 上已不成立（中间历经若干性能优化）。

**据此重新评估 PF1 的三条路径**：

| 路径 | 评估 | 结论 |
|---|---|---|
| ① 背板加脏标记（跳过重录） | 实测零收益，且引入**残影**风险（探针测不出、只能目视） | **不做** |
| ② 静止态层数 3→2 | 需改 `GlassSurface` 容器结构，与玻璃引擎铁律「容器结构不可改」冲突 | **不做** |
| ③ 降低模糊分辨率（重采样削减） | 唯一能真正动到瓶颈的手段，但模糊颗粒度会变、**属观感变化**，底栏是 v3.50.7 定稿区 | **需用户授权后评估** |

⇒ **在「观感必须不变」这一硬约束下，PF1 没有有效手段，建议暂缓。**
若用户接受观感变化，可只从 ③ 入手 —— 它收益直接，但等于要重新定稿底栏观感。
