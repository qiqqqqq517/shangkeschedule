...
interface CourseDao {
    ...
    @Query(
        """
        SELECT * FROM courses 
        WHERE courseTableId = :courseTableId AND isCrush = 0
        ORDER BY 
            day ASC, 
    ...
    /**
     * 删除指定课表ID下的所有本人课程（不含 crush 课程）。
     * 这是支持覆盖导入功能的关键方法。
     */
    @Query("DELETE FROM courses WHERE courseTableId = :courseTableId AND isCrush = 0")
     ...
     */
    @Query("DELETE FROM courses WHERE courseTableId = :tableId AND isCrush = 0 AND name IN (:courseNames)")
    ...
    /**
     * 获取指定课表在特定周次下的所有 crush 课程及其周数（整周数据）。
     * 用于情侣课表模式下整周数据的一次性加载。
     */
    @Transaction
    @Query(
    ...
    @Query(
        """
        SELECT * FROM courses 
        WHERE courseTableId = :courseTableId AND isCrush = 1
        ORDER BY 
            day ASC, 
            CASE WHEN isCustomTime = 0 THEN startSection ELSE 99 END ASC,  
            CASE WHEN isCustomTime = 1 THEN customStartTime ELSE '99:99' END ASC
    ...
    @Suppress(RoomWarnings.QUERY_MISMATCH)
    fun getCrushCoursesWithWeeksByTableId(courseTableId: String): Flow<List<CourseWithWeeks>>

    /**
     * 获取指定课表ID下，在特定星期和周次的所有 crush 课程及其周数。
    ...
    /**
     * 删除指定课表ID下的所有 crush 课程。
     * 用于「删除 crush 课表」功能，不影响本人课表数据。
     */
    @Query("DELETE FROM courses WHERE courseTableId = :courseTableId AND isCrush = 1")
    suspend fun deleteCrushCoursesByTableId(courseTableId: String)
}