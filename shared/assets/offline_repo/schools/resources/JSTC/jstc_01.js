...
async function runImportFlow() {
    window.shangkeBridge.showToast("开始拉取学期列表...");

    // 获取学期列表
    const semesters = await fetchSemesters();
    if (!semesters || semesters.length === 0) {
        window.shangkeBridge.showToast("获取学期列表失败，请检查登录状态或网络");
        return;
    }
    ...
    if (selectedIndex === null || selectedIndex < 0) {
        window.shangkeBridge.showToast("操作已取消");
        return;
    }
    ...
    // 并行获取元数据与课程表
    window.shangkeBridge.showToast("正在拉取课表数据...");
    ...
    if (!courses || courses.length === 0) {
        window.shangkeBridge.showToast("未查询到有效课程数据");
        return;
    }
    ...
    try {
        ...
        window.shangkeBridge.showToast("配置保存异常: " + e.message);
        return;
    }
    ...
    try {
        ...
        window.shangkeBridge.showToast("课程保存异常: " + e.message);
        return;
    }
    ...
    try {
        ...
        window.shangkeBridge.showToast("作息时间保存异常: " + e.message);
    }
