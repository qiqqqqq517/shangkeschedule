...
async function saveCourses(courses) {
    ...
    try {
        window.shangkeBridge.showToast('正在保存课程...');
        
        const result = await window.shangkeBridgePromise.saveImportedCourses(
...
async function saveTimeSlots() {
    ...
    try {
        window.shangkeBridge.showToast('正在保存时间段配置...');
        
        const timeSlots = generateTimeSlots();
        
        const result = await window.shangkeBridgePromise.savePresetTimeSlots(
...
async function runImportFlow() {
    ...
    try {
        ...
        window.shangkeBridge.showToast('课程表导入完成！');
...
if (isOnSchedulePage() || document.querySelector('table.CourseFormTable')) {
    ...
    window.shangkeBridge.showToast('请先在教务系统打开课程表页面！');
}
