...
// 桥接 API 使用 v2：window.shangkeBridge(同步) / window.shangkeBridgePromise(异步)
...
/**
 * 节次与周次合并去重函数（官方参考实现，来自上课 wiki《课程合并与去重函数》）
...
async function runImportFlow() {
    const BRIDGE = window.shangkeBridge;
    const BRIDGE_P = window.shangkeBridgePromise;
    if (!BRIDGE || !BRIDGE_P) {
        console.error("桥接 API 不可用（缺少 window.shangkeBridge / window.shangkeBridgePromise）");
        return;
    }
