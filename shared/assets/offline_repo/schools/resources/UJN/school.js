...
async function checkEnvirenment() {
  ...
  if (
    ...
    window.shangkeBridge.showToast("当前页面不在支持的导入范围内");
    const selectedOption =
      await window.shangkeBridgePromise.showSingleSelection(
...
async function runImportFlow() {
  ...
  try {
    ...
    window.shangkeBridge.showToast("保存时间段失败，请重试");
    return;
  }
  // 8. 流程**完全成功**，发送结束信号。
  window.shangkeBridge.showToast(`导入成功：共 ${savedCourseCount} 门课程`);
