...
if (_idx < 0) {
    window.shangkeBridge.showToast("请先进入(新)教务的「我的课表」页面再导入");
...
async function selectSemester() {
    ...
    if (current) {
        // 2. 问用户：导入当前学期 / 选择其他
        const choice = await window.shangkeBridgePromise.showSingleSelection(
            "选择学期",
            JSON.stringify([
                `导入当前学期：${current.mc}`,
                "选择其他学期"
    ...
    try {
        ...
        window.shangkeBridge.showToast("学期列表获取失败，请检查登录状态");
        console.error("学期列表获取失败:", e);
        return null;
    }
    if (rows.length === 0) {
        window.shangkeBridge.showToast("未获取到学期列表");
        return null;
    }
    const labels = rows.map(r => r.MC || r.DM);
    const idx = await window.shangkeBridgePromise.showSingleSelection(
        "选择其他学期",
...
async function saveAll(courses, timeSlots, startDate, totalWeeks) {
    ...
    const cfgOk = await window.shangkeBridgePromise.saveCourseConfig(JSON.stringify(config));
    if (!cfgOk) {
        window.shangkeBridge.showToast("课表配置保存失败");
        return false;
    }

    // 2. 节次时间
    const slotOk = await window.shangkeBridgePromise.savePresetTimeSlots(JSON.stringify(timeSlots));
    if (!slotOk) {
        window.shangkeBridge.showToast("节次时间保存失败");
        return false;
    }

    // 3. 课程
    const courseOk = await window.shangkeBridgePromise.saveImportedCourses(JSON.stringify(courses));
    if (!courseOk) {
        window.shangkeBridge.showToast("课程数据保存失败");
        return false;
    }
    return true;
}
...
async function runImportFlow() {
    window.shangkeBridge.showToast("吉林大学课表导入启动...");
    try {
        ...
        if (courses.length === 0) {
            window.shangkeBridge.showToast("该学期未查询到课程数据");
            return;
        }
        ...
        // 5. 保存
        window.shangkeBridge.showToast(`当前学期：${sem.mc}，${finalCourses.length} 条课程${changeNote ? "（" + changeNote + "）" : ""}，正在导入...`);
        ...
        if (undetermined.length > 0) {
            try {
                ...
                window.shangkeBridge.showToast(`导入成功！共 ${finalCourses.length} 条课程。`);
            }
        } else {
            window.shangkeBridge.showToast(`导入成功！共 ${finalCourses.length} 条课程。`);
        }
        ...
        window.shangkeBridge.showToast("意外错误: " + error.message);
    }
}
