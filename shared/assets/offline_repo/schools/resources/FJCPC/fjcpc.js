// 福建船政交通职业学院(fjcpc.edu.cn)上课适配脚本
...
async function fetchAndParseData() {
    ...
    if (!match) {
        window.shangkeBridge.showToast("未发现登录凭证，请登录后重试");
        return null;
    }
    ...
    try {
        ...
        window.shangkeBridge.showToast("数据解析失败: " + e.message);
        return null;
    }
}
...
async function runImportFlow() {
    window.shangkeBridge.showToast("课程导入流程开始...");
    ...
    if (timeSlots === null) {
        window.shangkeBridge.showToast("已取消选择作息。");
        return;
    }
    ...
    // 保存课表配置
    try {
        await window.shangkeBridgePromise.saveCourseConfig(JSON.stringify(data.config));
    } catch (e) {
        window.shangkeBridge.showToast("保存配置失败");
        return;
    }

    // 课程数据保存
    try {
        await window.shangkeBridgePromise.saveImportedCourses(JSON.stringify(data.courses));
        window.shangkeBridge.showToast(`成功抓取 ${data.courses.length} 门课程！`);
    } catch (e) {
        window.shangkeBridge.showToast("保存课程数据失败");
        return;
    }
    ...
    try {
        ...
        window.shangkeBridge.showToast("作息时间同步成功");
    } catch (e) {
