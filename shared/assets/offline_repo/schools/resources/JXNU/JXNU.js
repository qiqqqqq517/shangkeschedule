...
async function run() {
    try {
        ...
        if (!confirmed) { window.shangkeBridge.showToast("用户取消了导入。"); return; }

        const weeksInput = await getTotalWeeks();
        if (weeksInput === null) { window.shangkeBridge.showToast("导入已取消。"); return; }
        const totalWeeks = parseInt(weeksInput, 10);
        if (isNaN(totalWeeks) || totalWeeks < 1) { window.shangkeBridge.showToast("周数设置无效。"); return; }
        const defaultWeeks = Array.from({ length: totalWeeks }, (_, i) => i + 1);

        window.shangkeBridge.showToast("正在解析课表数据...");
        ...
        if (parsedCourses.length === 0) {
            ...
            await window.shangkeBridgePromise.showAlert("未解析到课程", detail, "确定");
            return;
        }
        ...
        try {
            await window.shangkeBridgePromise.saveImportedCourses(JSON.stringify(coursesWithWeeks));
            window.shangkeBridge.showToast(`课程数据已导入（共 ${coursesWithWeeks.length} 条）`);
        } catch (saveErr) {
            console.error("[JXNU] 保存课程失败:", saveErr);
            await window.shangkeBridgePromise.showAlert("保存课程失败", saveErr.message || String(saveErr), "确定");
            return;
        }
        ...
        try {
            await window.shangkeBridgePromise.showAlert("导入失败", `未知错误：${err.message || err}\n\n请联系开发者。`, "确定");
        } catch (_) {}
        window.shangkeBridge.notifyTaskCompletion();
    }
}
