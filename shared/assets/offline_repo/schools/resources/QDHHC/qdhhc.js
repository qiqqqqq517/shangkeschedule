...
function parseJsonData(jsonData) {
    ...
    // 调用合并与去重函数，整理连续节次与单双周周次
    const finalCourseList = mergeAndDistinctCourses(initialCourseList);
    
    console.log(`JS: JSON 数据解析与合并完成，整理后共 ${finalCourseList.length} 门课程。`);
...
async function fetchSemesterStartDate(academicYear, semesterCode) {
    const url = "http://jwxt.qdhhc.edu.cn/jwglxt/kbcx/xskbcxZccx_cxZcByXnxq.html?gnmkdm=N2154";
...
async function fetchAndParseCourses(academicYear, semesterIndex) {
    ...
    console.log("JS: 获取到的开学日期为:", semesterStartDate);

    const targetUrls = [
        "http://jwxt.qdhhc.edu.cn/jwglxt/kbcx/xskbcx_cxXsgrkb.html?gnmkdm=N2151"
    ];
    ...
    for (const url of targetUrls) {
        try {
            ...
            console.error(`JS: 接口请求失败: ${url}`, e);
        }
    }
    window.shangkeBridge.showToast("未能获取课表数据，请检查网络环境或登录状态。");
    return null;
}
...
async function runImportFlow() {
    ...
    try {
        ...
        window.shangkeBridge.showToast(configMsg);
    } catch (error) {
