...
async function fetchAndParseCourses(academicYear, semesterIndex) {
    ...
    try {
        ...
        try {
            ...
            window.shangkeBridge.showToast("数据解析失败，请检查是否登录过期。");
            return null;
        }
        ...
        if (courses.length === 0) {
            window.shangkeBridge.showToast("未找到课程数据，请检查学年学期。");
            return null;
        }
        ...
        window.shangkeBridge.showToast(`请求失败: ${error.message}`);
        console.error(error);
        return null;
    }
}
...
async function runImportFlow() {
    if (isLoginPage()) {
        window.shangkeBridge.showToast("导入失败：请先在浏览器登录教务系统！");
        return;
    }
