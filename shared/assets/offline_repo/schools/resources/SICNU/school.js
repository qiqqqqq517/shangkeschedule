...
async function main() {
    try {
        ...
        // 2. 获取课程数据
        window.shangkeBridge.showToast('正在采集课程数据...');
        ...
        // 7. 导入数据
        window.shangkeBridge.showToast(`正在导入 ${allCourses.length} 门课程...`);
        ...
        const timeSlotResult = await window.shangkeBridgePromise.savePresetTimeSlots(
            JSON.stringify(timeSlots)
        );
        console.log('[教务适配] 时间段导入结果:', timeSlotResult);

        const configResult = await window.shangkeBridgePromise.saveCourseConfig(
        ...
        window.shangkeBridge.showToast('课程数据采集完成！请保存下载的文件。');
        window.shangkeBridge.notifyTaskCompletion();

    } catch (error) {
        console.error('[教务适配] ❌ 执行失败:', error.message);
        console.error(error.stack);
        window.shangkeBridge.showToast('采集失败: ' + error.message);
    }
}
