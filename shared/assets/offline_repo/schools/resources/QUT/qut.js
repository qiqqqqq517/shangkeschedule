...
async function selectSemester() {
    ...
    var semesterIndex = await window.shangkeBridgePromise.showSingleSelection(
        "选择学期",
...
async function fetchAndParseCourses(academicYear, semesterIndex) {
    ...
    try {
        ...
        if (jsonText.indexOf("登录") !== -1 && jsonText.indexOf("密码") !== -1) {
            window.shangkeBridge.showToast("未登录或登录已过期，请先登录教务系统");
            return null;
        }
        ...
        if (courses.length === 0) {
            window.shangkeBridge.showToast("未找到任何课程数据，请检查所选学年学期是否正确或本学期无课。");
            return null;
        }
