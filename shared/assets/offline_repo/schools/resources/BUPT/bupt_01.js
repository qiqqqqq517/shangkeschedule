...
(function () {
    ...
    async function alertUser(title, message) {
        if (window.shangkeBridgePromise && window.shangkeBridgePromise.showAlert) {
            return await window.shangkeBridgePromise.showAlert(title, message, "确定");
        }
        alert(title + "\n" + message);
        return true;
    }
    ...
    async function saveToApp(courses, timeSlots) {
        ...
        if (window.shangkeBridgePromise && window.shangkeBridgePromise.saveCourseConfig) {
            await window.shangkeBridgePromise.saveCourseConfig(JSON.stringify(config));
        }
        if (timeSlots.length > 0 && window.shangkeBridgePromise && window.shangkeBridgePromise.savePresetTimeSlots) {
            await window.shangkeBridgePromise.savePresetTimeSlots(JSON.stringify(timeSlots));
        }

        if (window.shangkeBridgePromise && window.shangkeBridgePromise.saveImportedCourses) {
