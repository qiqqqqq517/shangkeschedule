...
(function () {
    ...
    function canShowSingleSelection() {
        return !!(
            ...
            typeof window.shangkeBridgePromise.showSingleSelection === "function"
        );
    }
    ...
    async function askChooseSemester(doc) {
        ...
        while (pageIndex >= 0 && pageIndex < pages.length) {
            ...
            let selectedIndex;
            try {
                selectedIndex = await window.shangkeBridgePromise.showSingleSelection(
                    pageIndex === 0
                        ? "请选择要导出的学年学期"
        ...
        try {
            ...
            if (result === true) {
                ...
                if (
                    semesterConfig &&
                    window.shangkeBridgePromise &&
                    typeof window.shangkeBridgePromise.saveCourseConfig === "function"
                ) {
                    await window.shangkeBridgePromise.saveCourseConfig(
                        JSON.stringify({
