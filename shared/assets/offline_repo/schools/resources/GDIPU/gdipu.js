...
const promptForStartDate = async () => {
    try {
        ...
        const startDate = await window.shangkeBridgePromise.showPrompt(
            "请输入学期开始日期",
        ...
        if (!/^\d{4}-\d{2}-\d{2}$/.test(startDate)) {
            window.shangkeBridge.showToast("日期格式不正确，请使用YYYY-MM-DD格式");
...
const saveSchedule = async (courses, courseConfig, timeSlots) => {
    try {
        await Promise.allSettled([
            window.shangkeBridgePromise.saveCourseConfig(JSON.stringify(courseConfig)),
            window.shangkeBridgePromise.saveImportedCourses(JSON.stringify(courses)),
            window.shangkeBridgePromise.savePresetTimeSlots(JSON.stringify(timeSlots))
        ]);
        
        window.shangkeBridge.showToast("课程表导入成功！");
        return true;
    } catch (error) {
        console.error("保存课程数据时出错:", error);
        window.shangkeBridge.showToast("课程表导入失败：" + error.message);
        return false;
    }
};
...
const fetchMultiWeekTimetable = async (startDate) => {
    ...
    window.shangkeBridge.showToast("正在获取课程表数据，请稍候...");
    ...
    while (true) {
        ...
        try {
            ...
            window.shangkeBridge.showToast(`第${weekCount}周获取失败，继续下一周`);
            requestDate = addDays(requestDate, 7);
            continue;
        }
    }
...
(async () => {
    try {
        window.shangkeBridge.showToast("正在启动GDIPU课程表导入...");
        ...
        if (courses.length === 0) {
            window.shangkeBridge.showToast("未找到任何课程信息");
        ...
        window.shangkeBridge.showToast("导入课程表失败：" + error.message);
    }
})();