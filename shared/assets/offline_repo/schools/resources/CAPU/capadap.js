...
async function promptUserToStart() {
    ...
    window.shangkeBridge.showToast("开始流程咯~");
    return true;
}
...
async function extractCourseTime() {
    try {
        ...
        window.shangkeBridge.showToast(`解析失败: ${error.message}`);
        return null;
    }
}
...
async function fetchAllRawData() {
    ...
    if (!rawArrangedList || rawArrangedList.length === 0) {
        window.shangkeBridge.showToast("未检测到当前学期的课程数据");
        return null;
    }
...
async function saveConfig(baseInfo) {
    ...
    try {
        const configSuccess = await window.shangkeBridgePromise.saveCourseConfig(JSON.stringify(configData));
        if (!configSuccess) {
            window.shangkeBridge.showToast("学期保存失败");
            return false;
        }
        return true;
    } catch (error) {
...
async function runImportFlow() {
    try {
        ...
        if (finalCourses.length === 0) {
            window.shangkeBridge.showToast("解析失败：未能提取到有效课程");
            return;
        }
        ...
        try {
            ...
            await window.shangkeBridgePromise.savePresetTimeSlots(slotJson);
        } catch (e) {
            console.error("时间段写入失败:", e);
            window.shangkeBridge.showToast("时间段保存失败");
            return;
        }

        // 保存课程数据
        const saveResult = await window.shangkeBridgePromise.saveImportedCourses(JSON.stringify(finalCourses));
        if (!saveResult) {
            window.shangkeBridge.showToast("课程数据保存失败");
            return;
        }

        window.shangkeBridge.showToast("Hi ~ 课表导入成功！");
