...
async function setStartDate(suggestedDate) {
    const dateSelection = await window.shangkeBridgePromise.showPrompt(
        "请确认学期起始日期",
...
async function selectTerms() {
    ...
    const termIndex = await window.shangkeBridgePromise.showSingleSelection(
        "请选择学期",
...
async function setYearAndTerm(currentYear, currentTerm) {
    ...
    if (!confirmed) {
        window.shangkeBridge.showToast("使用默认学年和学期进行导入。");
        return false;
    }
    return true;
}
...
async function runImportFlow() {
    ...
    if (needModify) {
        ...
        window.shangkeBridge.showToast(`当前年份与学期已修改为：${parseInt(currentYear) + 1980} 年，学期 ${currentTerm === 2 ? '夏' : currentTerm === 1 ? '春（第二学期）' : '秋（第一学期）'}`);
    }

    // 设置当前学年和学期
    const setResult = await setCurrentYearAndTerm(currentYear, currentTerm);
    if (!setResult) {
        window.shangkeBridge.showToast("设置学年和学期失败！");
        return;
    }
    ...
    if (!timeSlots || timeSlots.length === 0) {
        window.shangkeBridge.showToast("获取时间段信息失败，使用默认时间段！");
    }

    // 获取并转换课程表数据
    let courses = await convertToTargetFormat(timetableUrl);
    if (courses.length === 0) {
        window.shangkeBridge.showToast("获取课程表数据失败，请重试！");
        return;
    }
    ...
    try {
        ...
        window.shangkeBridge.showToast("未输入起始日期。");
    }
    ...
    try {
        ...
        window.shangkeBridge.showToast(`课程导入成功，共导入 ${coursesCount} 门课程！`);
    } catch (err) {
        console.error("课程导入失败:", err);
        window.shangkeBridge.showToast("课程导入失败：" + err.message);
        return;
    }
    ...
    try {
        ...
        window.shangkeBridge.showToast("时间段导入成功！");
    } catch (err) {
    ...
    // 提交课表配置
    try {
        await window.shangkeBridgePromise.saveCourseConfig(JSON.stringify(coursesConfig));
        window.shangkeBridge.showToast("课表配置保存成功！");
    } catch (err) {
        console.error("课表配置保存失败:", err);
        window.shangkeBridge.showToast("课表配置保存失败：" + err.message);
        return;
    }
