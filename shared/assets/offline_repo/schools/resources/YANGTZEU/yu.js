...
(function () {
    ...
    async function runImportFlow() {
        if (!window.shangkeBridgePromise) {
        ...
        window.shangkeBridge.showToast(`导入成功，共 ${courses.length} 条课程`);
    ...
    (async function bootstrap() {
        try {
            ...
            window.shangkeBridge.showToast(`导入失败：${error && error.message ? error.message : "请检查教务连接"}`);
        }
    })();
})();
