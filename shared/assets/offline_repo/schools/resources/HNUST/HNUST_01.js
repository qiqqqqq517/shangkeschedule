...
async function selectTimeSlotsType() {
    ...
    if (typeof window.shangkeBridgePromise === 'undefined') {
        return 0; 
    }
...
async function runImportFlow() {
    try {
        if (typeof window.shangkeBridge !== 'undefined') {
            window.shangkeBridge.showToast("正在获取课表数据，请稍候...");
        } else {
        ...
        if (semesters.length > 0 && typeof window.shangkeBridgePromise !== 'undefined') {
            let selectedIdx = await window.shangkeBridgePromise.showSingleSelection(
                "请选择要导入的学期", 
            ...
            if (selectedIdx === null) {
                window.shangkeBridge.showToast("已取消导入");
                return;
            }

            if (selectedIdx !== defaultIndex) {
                window.shangkeBridge.showToast(`正在获取 [${semesters[selectedIdx]}] 课表...`);
        ...
        if (courses.length === 0) {
            ...
            if (typeof window.shangkeBridgePromise !== 'undefined') {
                await window.shangkeBridgePromise.showAlert("提示", errMsg, "好的");
            } else {
                alert(errMsg);
            }
            return;
        ...
        if (timeSlotsIndex === null && typeof window.shangkeBridgePromise !== 'undefined') {
             window.shangkeBridge.showToast("已取消选择作息时间，终止导入");
             return;
        }
        ...
        // APP 环境，执行保存配置和作息时间
        const configSaved = await window.shangkeBridgePromise.saveCourseConfig(JSON.stringify(config));
        const timeSlotsSaved = await window.shangkeBridgePromise.savePresetTimeSlots(JSON.stringify(selectedTimeSlots));
        if (!configSaved || !timeSlotsSaved) {
            window.shangkeBridge.showToast("保存课表时间配置失败！");
        ...
        if (!saveResult) {
            window.shangkeBridge.showToast("保存课程失败，请重试！");
            return;
        }

        window.shangkeBridge.showToast(`成功导入 ${courses.length} 节课程及作息时间！`);
        window.shangkeBridge.notifyTaskCompletion();

    } catch (error) {
        if (typeof window.shangkeBridge !== 'undefined') {
            window.shangkeBridge.showToast("导入发生异常: " + error.message);
        } else {
