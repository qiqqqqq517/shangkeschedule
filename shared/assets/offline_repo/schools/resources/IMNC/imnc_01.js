...
async function runImportFlow() {
    try {
        window.shangkeBridge.showToast("开始解析课表...");
        ...
        if (!courses || courses.length === 0) {
            await window.shangkeBridgePromise.showAlert("错误", "未在当前页面找到课表数据，请确认是否处于课表页面，或联系适配开发者。", "好的");
            return;
        }
        ...
        window.shangkeBridge.showToast(`成功导入 ${courses.length} 门课程块！`);
        window.shangkeBridge.notifyTaskCompletion();
        
    } catch (error) {
        window.shangkeBridge.showToast("导入发生错误: " + error.message);
    }
}
