...
async function fetchAndParseCourses(xnxq) {
    window.shangkeBridge.showToast(`正在请求 ${xnxq} 的课表数据...`);
    ...
    try {
        ...
        if (jsonData.ret !== 0) {
             ...
             window.shangkeBridge.showToast(`数据请求失败: ${jsonData.msg}，请检查是否登录或参数是否正确。`);
             return null;
        }
...
async function runImportFlow() {
    ...
    if (selectedTimeSlots === null) {
        window.shangkeBridge.showToast("导入已取消，未选择作息时间。");
        return;
    }
