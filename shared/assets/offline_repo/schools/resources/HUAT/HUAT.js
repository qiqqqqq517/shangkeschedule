...
async function demoPrompt(semesterInfo) {
    try {
        ...
        const semesterStart = await window.shangkeBridgePromise.showPrompt(
            "📅 设置开学日期",
...
async function pickSemester(semesterList) {
    ...
    try {
        ...
        if (selectedIndex !== null && selectedIndex >= 0 && selectedIndex < semesterList.length) {
            ...
            window.shangkeBridge.showToast("已选 " + picked.yearTermName);
            return picked;
        }
        console.log("用户取消了学期选择。");
        window.shangkeBridge.showToast("学期选择：用户取消");
        return null;
    } catch (error) {
        console.error("显示学期选择弹窗出错:", error);
        window.shangkeBridge.showToast("学期选择出错：" + error.message);
        return null;
    }
}
...
async function importPresetTimeSlots() {
    ...
    try {
        ...
        if (selectedIndex !== null && selectedIndex >= 0 && selectedIndex < seasons.length) {
            ...
            window.shangkeBridge.showToast("你选择了 " + seasons[selectedIndex]);
            ...
            if (result === true) {
                ...
                window.shangkeBridge.showToast(`✅ ${seasons[selectedIndex]}时间段导入成功！`);
            ...
            console.log("用户取消了选择。");
            window.shangkeBridge.showToast("季节选择：用户取消了选择！");
            return false; // 用户取消时返回 false
        }
    } catch (error) {
        console.error("显示季节选择单选列表弹窗时发生错误:", error);
        window.shangkeBridge.showToast("季节选择：显示列表出错！" + error.message);
...
async function importSchedule(semesterInfo) {
    ...
    try {
        window.shangkeBridge.showToast("正在从教务系统拉取课表...");
        ...
        // 预览
        const preview = await window.shangkeBridgePromise.showAlert(
            "📊 数据预览",
        ...
        // 导入课程
        window.shangkeBridge.showToast("正在导入课程...");
        const saveResult = await window.shangkeBridgePromise.saveImportedCourses(JSON.stringify(courses));
        ...
        // 保存课表配置
        const configResult = await window.shangkeBridgePromise.saveCourseConfig(JSON.stringify({
...
async function showFinalResult(results) {
    ...
    try {
        await window.shangkeBridgePromise.showAlert(title, msg, '知道了');
    } catch (e) {
        console.error('显示最终结果弹窗失败:', e);
        window.shangkeBridge.showToast(title);
    }
}

async function runAllDemosSequentially() {
    window.shangkeBridge.showToast("🚀 课表导入助手启动...");
    ...
    try {
        ...
        let semesterList = [];
        try {
            window.shangkeBridge.showToast("正在获取学期列表...");
