...
`;
    ...
    if (confirmed) {
        if (currentHost == wspc) {
        window.shangkeBridge.showToast("请先登录");
        return false;
        }
            ...
            window.shangkeBridge.showToast("请在学校的网站内导入");
        }
    else {
        window.shangkeBridge.showToast("取消导入操作");
        return false;
    }
}
...
async function runImportFlow() {
    ...
    if(rawCourses.length === 0){
        window.shangkeBridge.showToast("未找到任何课程! 请确认已登录并进入课表页面。");
        return;
    }
    const mergedCourses = mergeCourses(rawCourses);
    if(mergedCourses.length === 0){
        window.shangkeBridge.showToast("课程合并失败，未生成任何课程数据！");
        return;
    }
    const finalCourses = sortCourses(mergedCourses);
    if(finalCourses.length === 0){
        window.shangkeBridge.showToast("课程排序失败，未生成任何课程数据！");
        return;
    } 
    // 生成时间段数据
    const timeSlots = generateTimeSlots();
    if(timeSlots.length === 0){
        window.shangkeBridge.showToast("时间段生成失败，未生成任何时间段数据！");
        return;
    }
    // 生成配置数据
    ...
    window.shangkeBridge.showToast("成功导入课表！请前往设置调整开学时间等信息！");
