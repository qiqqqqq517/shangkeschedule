// 金智(Wisedu)教务系统通用适配器（DOM 矩阵解析版）
// 自动识别课表表格：星期表头 + 节次行，支持 rowspan/colspan 与 iframe 内课表。
(function () {
    'use strict';

    var PLATFORM_NAME = '金智(Wisedu)教务系统';
    var PREFERRED_SELECTORS = ['#kbTable','.kb_tb','.scheduleTable','table[class*=kb]'];
    var BODY_KEYWORDS = ['金智','kcbxx','kbxx'];
    var URL_PATTERNS = [
        /wisedu/g,
        /jwapp/g,
        /ehall/g,
    ...
    function parseJsonWeekList(value) {
        if (!value && value !== 0) return [];
        if (Object.prototype.toString.call(value) === '[object Array]') {
            return value.map(function (n) { return parseInt(n, 10); })
                .filter(function (n) { return n >= 1 && n <= 32; })
                .sort(function (a, b) { return a - b; });
        }
        var s = String(value);
        if (s.indexOf('周') !== -1) return parseWeeks(s);
        var weeks = {};
        var found = false;
        var re = /(\d+)\s*(?:[-~－—至到]\s*(\d+))?/g;
        var m;
    ...
    function parseJsonSections(value) {
        var nums = String(value == null ? '' : value).split(/[,，\-~－—至到]+/)
    ...
    function extractEmbeddedJsonCourses(doc) {
        try {
            var html = (doc.body && doc.body.innerHTML) || '';
            if (!html) return null;
            var match = html.match(/var\s+(?:kcbxx|kbxx)\s*=\s*(\[[\s\S]*?\])\s*;/);
            if (!match) {
                var scripts = doc.querySelectorAll('script');
                for (var i = 0; i < scripts.length; i++) {
                    var m2 = (scripts[i].textContent || '').match(/var\s+(?:kcbxx|kbxx)\s*=\s*(\[[\s\S]*?\])\s*;/);
                    if (m2) { match = m2; break; }
                }
            }
            if (!match || !match[1]) return null;
            var raw = [];
            try { raw = JSON.parse(match[1]); } catch (e) { return null; }
            if (!raw || !raw.length) return null;
            var courses = [];
            for (var j = 0; j < raw.length; j++) {
                var item = raw[j] || {};
                var name = stripHtml(firstDefined(item, ['kcmc', 'kcm', 'courseName', 'name']));
                var teacher = stripHtml(cleanTeacher(firstDefined(item, ['teaxms', 'tmc', 'xm', 'teacher', 'js'])));
                var position = stripHtml(firstDefined(item, ['jxcdmcs', 'cdmc', 'jxdd', 'position']));
                var day = parseInt(firstDefined(item, ['xq', 'xqj', 'day']));
                var weeks = parseJsonWeekList(firstDefined(item, ['zcs', 'zcd', 'weeks']));
                var sections = parseJsonSections(firstDefined(item, ['jcdm2', 'jcs', 'jc', 'djj', 'sections']));
                if (!name || isNaN(day) || day < 1 || day > 7 || !sections || !weeks.length) continue;
                courses.push({
                    name: name,
                    teacher: teacher || '未知',
                    position: position || '待定',
    ...
    async function saveAndFinish(courses) {
    ...
    async function runImport() {
        try {
            var jsonCourses = extractEmbeddedJsonCourses(document);
            if (jsonCourses && jsonCourses.length) {
                await saveAndFinish(jsonCourses);
                return;
            }

            var found = findScheduleTable();
            if (!found.table) {
                showToast('未找到课表表格，请进入金智(Wisedu)教务系统的学生个人课表页面并等待表格加载后再试');
                return;
            }
    ...
    if (matchesPlatformHint()) {
        showToast('检测到金智(Wisedu)教务系统，点击导入按钮抓取课表');
    }
})();
