...
function parseCourseTable(htmlContent) {
    ...
    if (!table) {
        window.shangkeBridge.showToast("错误：未找到课表表格 (id=kbtable)。");
        return [];
    }
...
async function fetchCourseHtml() {
    ...
    try {
        ...
        window.shangkeBridge.showToast("课表数据获取成功，开始解析...");
        return text;
    } catch (error) {
        window.shangkeBridge.showToast(`网络请求异常: ${error.message}`);
        return null;
    }
}
...
async function saveCourses(parsedCourses, originalCount, mergedCount) {
    if (parsedCourses.length === 0) {
        window.shangkeBridge.showToast("未解析到任何课程数据，跳过保存。");
        return true;
    }
    ...
    try {
        ...
        if (originalCount !== undefined && mergedCount !== undefined) {
             window.shangkeBridge.showToast(`课程导入成功！原始 ${originalCount} 条，去重合并 ${mergedCount} 条，最终导入 ${parsedCourses.length} 条。`);
        } else {
             window.shangkeBridge.showToast(`成功导入 ${parsedCourses.length} 条课程！`);
        }
        return true;
    } catch (error) {
        window.shangkeBridge.showToast(`保存失败: ${error.message}`);
        return false;
    }
}
...
async function runImportFlow() {
    ...
    if (htmlContent === null) {
        window.shangkeBridge.showToast("导入终止。");
        return; 
    }
    ...
    if (parsedCourses.length === 0) {
        window.shangkeBridge.showToast("解析失败或未发现有效课程。导入终止。");
        return;
    }
    ...
    // 流程成功
    window.shangkeBridge.showToast("所有任务已完成！课表已导入成功！");
