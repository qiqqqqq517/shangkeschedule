...
function parseRawScheduleData(rawData) {
    window.shangkeBridge.showToast("正在解析原始数据...");
...
async function fetchAndParseData(shareKey) {
    try {
        window.shangkeBridge.showToast("正在通过 WakeUP 官方渠道请求课表数据...");
        ...
        window.shangkeBridge.showToast(`数据解析成功，共 ${courses.length} 门课程`);
        return { timeSlots, courseConfig, courses };
    } catch (error) {
        console.error("WakeUP 数据获取或解析失败:", error);
        window.shangkeBridge.showToast(`数据获取或解析失败: ${error.message}`);
        return null;
    }
}
...
async function saveTimeSlots(timeSlots) {
    if (timeSlots.length === 0) {
        window.shangkeBridge.showToast("没有可导入的时间段数据。");
        return true;
    }
    try {
        console.log("正在导入时间段...");
        await window.shangkeBridgePromise.savePresetTimeSlots(JSON.stringify(timeSlots));
        window.shangkeBridge.showToast(`成功导入 ${timeSlots.length} 个时间段！`);
        return true;
    } catch (error) {
...
async function saveConfig(configData) {
    try {
        ...
        window.shangkeBridge.showToast("课表配置（学期/时长）更新成功！");
        return true;
    } catch (error) {
...
async function saveCourses(courses) {
    if (courses.length === 0) {
        window.shangkeBridge.showToast("没有课程数据需要导入。");
        return true;
    }
    try {
...
async function runImportFlow() {
    ...
    window.shangkeBridge.showToast("课表导入流程即将开始...");

    // 获取用户输入
    const userInput = await window.shangkeBridgePromise.showPrompt(
        "输入课表分享口令",
    ...
    // 流程完全成功，发送结束信号
    window.shangkeBridge.showToast("所有任务已成功完成！");
