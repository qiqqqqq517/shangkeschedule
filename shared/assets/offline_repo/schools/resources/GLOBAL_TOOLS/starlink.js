...
async function runStarlinkImport() {
    try {
        ...
        window.shangkeBridge.showToast("正在同步云端数据...");
        ...
        const success = await window.shangkeBridgePromise.saveImportedCourses(JSON.stringify(finalCourses));

        if (success) {
            window.shangkeBridge.showToast("导入成功！");
            window.shangkeBridge.notifyTaskCompletion();
        }
    } catch (e) {
        await window.shangkeBridgePromise.showAlert("导入失败", e.message, "确定");
    }
}
