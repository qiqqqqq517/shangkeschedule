...
async function demoAlert() {
    try {
        ...
        if (confirmed) {
            ...
            shangkeBridge.showToast("Alert：用户点击了确认！");
            return true; // 成功时返回 true
        } else {
            console.log("用户点击了取消按钮或关闭了弹窗。Alert Promise Resolved: " + confirmed);
            shangkeBridge.showToast("Alert：用户取消了！");
            return false; // 用户取消时返回 false
        }
    } catch (error) {
        console.error("显示公告弹窗时发生错误:", error);
        shangkeBridge.showToast("Alert：显示弹窗出错！" + error.message);
...
async function demoPrompt() {
    try {
        ...
        if (name !== null) {
            ...
            shangkeBridge.showToast("欢迎你，" + name + "！");
            return true; // 成功时返回 true
        } else {
            console.log("用户取消了输入。");
            shangkeBridge.showToast("Prompt：用户取消了输入！");
            return false; // 用户取消时返回 false
        }
    } catch (error) {
        console.error("显示输入框弹窗时发生错误:", error);
        shangkeBridge.showToast("Prompt：显示输入框出错！" + error.message);
        return false; // 出现错误时也返回 false
    }
}
...
async function demoSingleSelection() {
    ...
    try {
        ...
        if (selectedIndex !== null && selectedIndex >= 0 && selectedIndex < fruits.length) {
            ...
            shangkeBridge.showToast("你选择了 " + fruits[selectedIndex]);
            return true; // 成功时返回 true
        } else {
            console.log("用户取消了选择。");
            shangkeBridge.showToast("Single Selection：用户取消了选择！");
            return false; // 用户取消时返回 false
        }
    } catch (error) {
        console.error("显示单选列表弹窗时发生错误:", error);
        shangkeBridge.showToast("Single Selection：显示列表出错！" + error.message);
        return false; // 出现错误时也返回 false
    }
}
...
async function demoSaveCourses() {
    ...
    try {
        ...
        if (result === true) {
            console.log("课程导入成功！");
            shangkeBridge.showToast("测试课程导入成功！");
        } else {
            console.log("课程导入未成功，结果：" + result);
            shangkeBridge.showToast("测试课程导入失败，请查看日志。");
        }
    } catch (error) {
        console.error("导入课程时发生错误:", error);
        shangkeBridge.showToast("导入课程失败: " + error.message);
    }
}
...
async function demoSaveConfig() {
    ...
    try {
        ...
        if (result === true) {
            ...
            shangkeBridge.showToast("测试配置导入成功");
        } else {
            console.log("课表配置导入未成功，结果：" + result);
            shangkeBridge.showToast("测试配置导入失败，请查看日志。");
        }
    } catch (error) {
        console.error("导入配置时发生错误:", error);
        shangkeBridge.showToast("导入配置失败: " + error.message);
    }
}


shangkeBridge.showToast("这是一个来自 JS 的 Toast 消息，会很快消失！");

/**
 * 编排这些异步操作，并在用户取消时停止后续执行。
 */
async function runAllDemosSequentially() {
    shangkeBridge.showToast("所有演示将按顺序开始...");
    ...
    shangkeBridge.showToast("所有弹窗演示已完成！");
