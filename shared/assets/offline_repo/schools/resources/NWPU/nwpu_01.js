...
async function getCourseData(semesterId, studentId) {
    ...
    try {
        ...
        window.shangkeBridge.showToast("获取课程数据失败: " + e.message);
        return null;
    }
}
...
async function saveTimeSlots(timeSlots) {
    ...
    try {
        ...
        window.shangkeBridge.showToast("保存时间段失败: " + e.message);
        return false;
    }
}
...
async function saveConfig(config) {
    try {
        ...
        window.shangkeBridge.showToast("课表配置导入成功");
        return true;
    } catch (e) {
        console.error("[NWPU] 保存配置失败: " + e.message);
        window.shangkeBridge.showToast("保存配置失败: " + e.message);
        return false;
    }
}
...
async function saveCourses(courses) {
    ...
    try {
        ...
        window.shangkeBridge.showToast("保存课程失败: " + e.message);
        return false;
    }
}
...
async function runImportFlow() {
    try {
        // 1. 显示导入说明
        var confirmed = await window.shangkeBridgePromise.showAlert(
            "西北工业大学课表导入",
        ...
        while (true) {
            ...
            // 无数据，弹窗提示后重新选择
            var retry = await window.shangkeBridgePromise.showAlert(
                "无课程数据",
        ...
        // 5. 获取学期信息（开始日期、结束日期）
        window.shangkeBridge.showToast("正在获取学期信息...");
        ...
        window.shangkeBridge.showToast("导入异常: " + e.message);
    }
}
