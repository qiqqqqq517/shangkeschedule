...
(function () {
    ...
    async function saveImportedData(courses) {
        const courseResult = await window.shangkeBridgePromise.saveImportedCourses(JSON.stringify(courses));
        if (courseResult !== true) {
            throw new Error('课程保存失败：' + courseResult);
        }

        if (typeof window.shangkeBridgePromise.saveCourseConfig === 'function') {
