...
    const result = await window.shangkeBridgePromise.saveImportedCourses(JSON.stringify(finalCourses));
    if (result !== true) {
...
    const result = await window.shangkeBridgePromise.savePresetTimeSlots(JSON.stringify(generatedSlots));
    if (result !== true) {
...
    let startDate = await window.shangkeBridgePromise.showPrompt(
        "输入开学日期", 
