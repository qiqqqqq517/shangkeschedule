...
async function getSemesterIndex() {
    try {
        const response = await fetch(`https://bk.cup.edu.cn/student/for-std/course-table`, { credentials: 'include' })
        ...
        if (selectedIndex !== null && selectedIndex >= 0) {
            ...
            if (typeof window.shangkeBridge !== 'undefined' && window.shangkeBridge.showToast) {
                window.shangkeBridge.showToast("已选择学期: " + semesterTexts[selectedIndex]);
            }
        ...
        window.shangkeBridge.showToast("Alert：获取学期信息出错！" + error.message);
...
async function fetchPrintData(semesterIndex) {
    try {
        const responds = await fetch(`https://bk.cup.edu.cn/student/for-std/course-table/semester/${semesterIndex}/print-data`, { credentials: 'include' });
        if (!responds.ok) {
        ...
        window.shangkeBridge.showToast("Alert：获取数据出错！" + error.message);
        return null;
    }
}
...
async function parseCourses(printData) {
    ...
    try {
        ...
        const result = await window.shangkeBridgePromise.saveImportedCourses(JSON.stringify(parsedCourses));
        if (result === true) {
            console.log("课程导入成功！");
...
async function saveConfig(semesterIndex) {
    ...
    const responds = await fetch(`https://bk.cup.edu.cn/student/ws/semester/get/${semesterIndex}`, { credentials: 'include' });
    if (!responds.ok) {
        throw new Error(`网络请求失败，状态码: ${responds.status}`);
    }
    ...
    try {
        ...
        const result = await window.shangkeBridgePromise.saveCourseConfig(configJsonString);

        if (result === true) {
            console.log("课表配置导入成功！");
            window.shangkeBridge.showToast("测试配置导入成功！开学日期: " + startDate.toISOString().split('T')[0]);
        } else {
            console.log("课表配置导入未成功，结果：" + result);
            window.shangkeBridge.showToast("测试配置导入失败，请查看日志。");
        }
        return result; // 返回导入结果，供流程控制使用
    } catch (error) {
        console.error("导入配置时发生错误:", error);
        window.shangkeBridge.showToast("导入配置失败: " + error.message);
    }
}
...
async function runImportFlow() {
    window.shangkeBridge.showToast("课程导入流程即将开始...");
    ...
    // 7. 流程**完全成功**，发送结束信号。
    window.shangkeBridge.showToast("所有任务已完成！");
