...
async function runImportFlow() {
    try {
        ...
        const year = await window.shangkeBridgePromise.showPrompt("选择学年", "请输入要导入的起始学年（例如 2025-2026 应输入2025）:", "", "validateYearInput");
        if (!year) return;

        const semesterIdx = await window.shangkeBridgePromise.showSingleSelection("选择学期", JSON.stringify(["第一学期", "第二学期"]), -1);
        ...
        if (courses.length === 0) {
            window.shangkeBridge.showToast("未找到课程，请检查学年学期选择或登录状态。");
            return;
        }

        await window.shangkeBridgePromise.saveCourseConfig(JSON.stringify({ semesterTotalWeeks: 20, firstDayOfWeek: 1 }));
        ...
        window.shangkeBridge.showToast("错误: " + err.message);
    }
}
