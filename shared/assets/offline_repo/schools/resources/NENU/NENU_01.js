...
async function runImportFlow() {
    try {
        if (typeof window.shangkeBridge !== 'undefined') {
            window.shangkeBridge.showToast("正在获取作息时间与学期列表...");
        } else {
        ...
        if (typeof window.shangkeBridgePromise !== 'undefined') {
            let userChoice = await window.shangkeBridgePromise.showSingleSelection(
        ...
        if (typeof window.shangkeBridge !== 'undefined') {
            window.shangkeBridge.showToast(`正在获取 [${semesters[selectedIdx]}] 数据...`);
        }
        ...
        await window.shangkeBridgePromise.savePresetTimeSlots(JSON.stringify(standardTimeSlots));
        
        const saveResult = await window.shangkeBridgePromise.saveImportedCourses(JSON.stringify(uniqueCourses));
        if (!saveResult) {
            window.shangkeBridge.showToast("保存失败，请重试！");
            return;
        }

        window.shangkeBridge.showToast(`成功导入 ${uniqueCourses.length} 节课程！`);
        window.shangkeBridge.notifyTaskCompletion();

    } catch (error) {
        if (typeof window.shangkeBridge !== 'undefined') window.shangkeBridge.showToast("异常: " + error.message);
