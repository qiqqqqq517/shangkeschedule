...
if (!VPN_BASE) {
    window.shangkeBridge.showToast("请通过扬州大学 WebVPN 进入教务系统后使用");
...
async function runImportFlow() {
    try {
        ...
        if (!select || select.options.length === 0) {
            window.shangkeBridge.showToast("未找到学期选择框，请先进入课表页面");
            return;
        }
        const texts = [];
        const values = [];
        ...
        window.shangkeBridge.showToast("正在获取教务数据...");
        ...
        if (courses.length === 0) {
            window.shangkeBridge.showToast("未解析到课程数据，请确认所选学期有课");
            return;
        }

        const timeSlots = parseTimeSlots(data);
        const ok = await window.shangkeBridgePromise.saveImportedCourses(JSON.stringify(courses));
        if (!ok) {
            window.shangkeBridge.showToast("课程保存失败");
            return;
        }
        await window.shangkeBridgePromise.savePresetTimeSlots(JSON.stringify(timeSlots.length ? timeSlots : FALLBACK_TIME_SLOTS));
        await window.shangkeBridgePromise.saveCourseConfig(JSON.stringify({ semesterTotalWeeks: 20 }));

        window.shangkeBridge.showToast("成功导入 " + courses.length + " 个课程时段");
        window.shangkeBridge.notifyTaskCompletion();
    } catch (e) {
        console.error("导入异常:", e);
        window.shangkeBridge.showToast("导入失败: " + e.message);
    }
}
