...
function parseSchedule() {
  ...
  if (!table) {
    window.shangkeBridge.showToast("请去往学期理论课表界面");
    return [];
  }
  ...
  if (!tbody) {
    ...
    window.shangkeBridge.showToast("课表结构异常");
    return [];
  }
...
(async () => {
  if (!checkLogin()) {
    window.shangkeBridge.showToast("尚未登录，请先登录！");
    return;
  }
  ...
  if (courses.length === 0) {
    window.shangkeBridge.showToast("未解析到任何课程");
    return;
  }
  ...
  window.shangkeBridge.showToast(`导入成功！${courses.length}门课程`);
