...
async function fetchAndParseCourses(academicYear, semesterIndex) {
    ...
    try {
        ...
        if (!response.ok) {
            window.shangkeBridge.showToast(`课表请求失败：HTTP ${response.status}`);
        ...
        if (!jsonData || !Array.isArray(jsonData.kbList) || jsonData.kbList.length === 0) {
            window.shangkeBridge.showToast("未查询到课表数据，请检查学年/学期是否选择正确，或确认已登录教务系统。");
            return null;
        }

        const parsedCourses = parseJsonData(jsonData);
        if (parsedCourses.length === 0) {
            window.shangkeBridge.showToast("课表数据为空或解析失败，请确认所选学年学期。");
            return null;
        }
        ...
        window.shangkeBridge.showToast("获取课表失败，请确认已登录教务系统且网络可访问 jw.gdou.edu.cn。");
        return null;
    }
}
