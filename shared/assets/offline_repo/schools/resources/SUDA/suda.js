...
async function runImportFlow() {
    ...
    // 2. 查找课表
    window.shangkeBridge.showToast("正在查找课表...");
    const table = findScheduleTable();
    if (!table) {
        window.shangkeBridge.showToast("未找到课表，请先打开「学生个人课表」页面。");
        return;
    }
    // 3. 解析课程
    ...
    if (courses.length === 0) {
        window.shangkeBridge.showToast("未解析到任何课程，请确认课表已正确加载。");
        return;
    }
    // 4. 保存课程
    window.shangkeBridge.showToast(`正在保存 ${courses.length} 条课程...`);
    await window.shangkeBridgePromise.saveImportedCourses(JSON.stringify(courses));
    // 5. 保存作息时间
    window.shangkeBridge.showToast(`正在导入 ${TimeSlots.length} 个时间段...`);
    ...
    // 7. 完成
    window.shangkeBridge.showToast(`课程导入成功，共导入 ${courses.length} 条课程！`);
