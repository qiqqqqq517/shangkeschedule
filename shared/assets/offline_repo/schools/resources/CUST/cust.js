                    ...
                    content.forEach(item => {
                        switch(item.Key) {
                            ...
                            case 'Time':
                                ...
                                break;
                            default:
                                // 收集其他字段（学分、考核方式、课程性质等）到备注
                                extraInfo.push(`${item.Key}：${item.Name}`);
                                break;
                        }
                    });
                    ...
                    if (courseName && weeks.length > 0) {
                        ...
                        if (coursesMap.has(courseKey)) {
                            ...
                            coursesMap.set(courseKey, {
                                ...
                                endSection: timePiece.EndSection,
                                weeks: weeks,
                                remark: extraInfo.length > 0 ? extraInfo.join('；') : undefined
                            });
                        }
                    }
                });
...
async function fetchScheduleData() {
    try {
        ...
        window.shangkeBridge.showToast('获取课表失败: ' + error.message);
        return null;
    }
}
...
async function importCourseSchedule() {
    try {
        console.log('开始导入课程表...');
        window.shangkeBridge.showToast('正在获取课表数据...');
        ...
        if (courses.length === 0) {
            window.shangkeBridge.showToast('未找到课程数据');
            return false;
        }
        ...
        // 导入课程
        const coursesResult = await window.shangkeBridgePromise.saveImportedCourses(JSON.stringify(courses));
        if (coursesResult === true) {
            console.log('课程导入成功！');
            window.shangkeBridge.showToast(`成功导入 ${courses.length} 门课程！`);
        } else {
            console.log('课程导入失败');
            window.shangkeBridge.showToast('课程导入失败');
            return false;
        }
        ...
        const timeSlotsResult = await window.shangkeBridgePromise.savePresetTimeSlots(JSON.stringify(finalTimeSlots));
        if (timeSlotsResult === true) {
            console.log('时间段导入成功！');
            window.shangkeBridge.showToast('时间段配置成功！');
        } else {
            console.log('时间段导入失败');
            window.shangkeBridge.showToast('时间段配置失败');
        }
...
async function demoAlert() {
    try {
        ...
        const confirmed = await window.shangkeBridgePromise.showAlert(
            "重要通知",
            "这是一个弹窗示例。",
            "好的"
        );
        if (confirmed) {
            ...
            window.shangkeBridge.showToast("Alert：用户点击了确认！");
            return true; // 成功时返回 true
        } else {
            console.log("用户点击了取消按钮或关闭了弹窗。Alert Promise Resolved: " + confirmed);
            window.shangkeBridge.showToast("Alert：用户取消了！");
            return false; // 用户取消时返回 false
        }
    } catch (error) {
        console.error("显示公告弹窗时发生错误:", error);
        window.shangkeBridge.showToast("Alert：显示弹窗出错！" + error.message);
...
async function demoPrompt() {
    try {
        ...
        const name = await window.shangkeBridgePromise.showPrompt(
            "输入你的姓名",
            "请输入至少2个字符",
            "测试用户",
            "validateName"
        ...
        if (name !== null) {
            ...
            window.shangkeBridge.showToast("欢迎你，" + name + "！");
            return true; // 成功时返回 true
        } else {
            console.log("用户取消了输入。");
            window.shangkeBridge.showToast("Prompt：用户取消了输入！");
            return false; // 用户取消时返回 false
        }
    } catch (error) {
        console.error("显示输入框弹窗时发生错误:", error);
        window.shangkeBridge.showToast("Prompt：显示输入框出错！" + error.message);
        return false; // 出现错误时也返回 false
    }
}
...
async function demoSingleSelection() {
    ...
    try {
        ...
        const selectedIndex = await window.shangkeBridgePromise.showSingleSelection(
            "选择你喜欢的水果",
        ...
        if (selectedIndex !== null && selectedIndex >= 0 && selectedIndex < fruits.length) {
            ...
            window.shangkeBridge.showToast("你选择了 " + fruits[selectedIndex]);
            return true; // 成功时返回 true
        } else {
            console.log("用户取消了选择。");
            window.shangkeBridge.showToast("Single Selection：用户取消了选择！");
            return false; // 用户取消时返回 false
        }
    } catch (error) {
        console.error("显示单选列表弹窗时发生错误:", error);
        window.shangkeBridge.showToast("Single Selection：显示列表出错！" + error.message);
        return false; // 出现错误时也返回 false
    }
}

// 仍然可以使用原始的 AndroidBridge 对象
window.shangkeBridge.showToast("这是一个来自 JS 的 Toast 消息，会很快消失！");
...
async function demoSaveCourses() {
    ...
    try {
        ...
        const result = await window.shangkeBridgePromise.saveImportedCourses(JSON.stringify(testCourses));
        if (result === true) {
            console.log("课程导入成功！");
            window.shangkeBridge.showToast("测试课程导入成功！");
        } else {
            console.log("课程导入未成功，结果：" + result);
            window.shangkeBridge.showToast("测试课程导入失败，请查看日志。");
        }
    } catch (error) {
        console.error("导入课程时发生错误:", error);
        window.shangkeBridge.showToast("导入课程失败: " + error.message);
    }
}
...
async function importPresetTimeSlots() {
    ...
    try {
        ...
        const result = await window.shangkeBridgePromise.savePresetTimeSlots(JSON.stringify(presetTimeSlots));
        if (result === true) {
            console.log("预设时间段导入成功！");
            window.shangkeBridge.showToast("测试时间段导入成功！");
        } else {
            console.log("预设时间段导入未成功，结果：" + result);
            window.shangkeBridge.showToast("测试时间段导入失败，请查看日志。");
        }
    } catch (error) {
        console.error("导入时间段时发生错误:", error);
        window.shangkeBridge.showToast("导入时间段失败: " + error.message);
    }
}
...
async function runAllDemosSequentially() {
    window.shangkeBridge.showToast("所有演示将按顺序开始...");
    ...
    window.shangkeBridge.showToast("所有弹窗演示已完成！");
...
if (isOnStudentPage()) {
    ...
    window.shangkeBridge.showToast('正在准备导入课程表...');
    ...
    window.shangkeBridge.showToast('请先登录教务系统！');
