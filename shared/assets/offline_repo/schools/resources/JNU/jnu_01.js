...
async function confirmAlert() {
    try {
        ...
        if (confirmed) {
            window.shangkeBridge.showToast("正在尝试导入课表...");
            return true; // 成功时返回 true
        } else {
            window.shangkeBridge.showToast("已取消！");
            return false; // 用户取消时返回 false
        }
    } catch (error) {
        window.shangkeBridge.showToast("显示弹窗出错！" + error.message);
...
async function getSession() {
    try {
        ...
        if (selectedIndex !== null && selectedIndex >= 0 && selectedIndex < yearArr.length) {
            ...
            window.shangkeBridge.showToast("用户取消了选择！");
            return -1; // 用户取消时返回-1
        }
    } catch (error) {
        window.shangkeBridge.showToast("显示列表出错！" + error.message);
        return -1; // 出现错误时也返回-1
    }
}
...
async function importPresetTimeSlots() {
    ...
    try {
        const result = await window.shangkeBridgePromise.savePresetTimeSlots(JSON.stringify(timeSlots));
        if (result === true) {
        } else {
            window.shangkeBridge.showToast("时间段导入失败!");
        }
    } catch (error) {
...
async function getCourses(xnxqID) {
    ...
    try {
        ...
        if (courseData.datas.xskcb.totalSize === 0){
            window.shangkeBridge.showToast("获取到的课表为空课表，请检查学年学期信息！")
            return -1;
        }
        window.shangkeBridge.showToast("成功获取课程表信息，共" + courseData.datas.xskcb.totalSize + "门课程");
        return courseData;
    }catch (error){
        window.shangkeBridge.showToast("获取课程表失败:" + error.message);
        return -1;
    }
}
...
async function saveCourses(courseData) {
    ...
    if (xnxqID === -1) {
        ...
        if(await confirmGetXNXQAlert()){
            ...
            window.shangkeBridge.showToast("你选择的学年学期信息：" + xnxqID);
        }else {
            return -1;
        }
    }else{
        window.shangkeBridge.showToast("自动获取到学年学期信息：" + xnxqID);
    }
    ...
    try {
        const result = await window.shangkeBridgePromise.saveImportedCourses(JSON.stringify(courses));
        if (result === true) {
            //课程导入成功！
...
async function saveConfig(semesterTotalWeeks) {
    ...
    try {
        ...
        if (result === true) {
            ...
            window.shangkeBridge.showToast("课表配置导入失败");
            return -1;
        }
    } catch (error) {
...
async function runAllDemosSequentially() {
    ...
    if(currentPageNum === 1){
        window.shangkeBridge.showToast("请登录教务系统");
        return;
    }else if(currentPageNum === 2) {
        window.shangkeBridge.showToast("请进入我的课表界面");
        return;
    }else if(currentPageNum === -1) {
        window.shangkeBridge.showToast("未知错误");
        return;
    }
