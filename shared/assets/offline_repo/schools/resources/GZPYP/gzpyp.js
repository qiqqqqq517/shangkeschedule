// 广州番禺职业技术学院（gzpyp）教务专属适配器
//
// ===== 站点事实（2026-09-16 实测）=====
//   登录：https://jiaowu.gzpyp.edu.cn/ （湖南强智经典登录页，图形验证码，无法自动登录）
//   教务：https://jiaowu.gzpyp.edu.cn/jsxsd/
//   课表页：https://jiaowu.gzpyp.edu.cn/jsxsd/xskb/xskb_list.do?viweType=0
//     表格 table#kbtable；第 0 行表头（空 | 星期一..星期日）。
//   行首节次标签：第1-2节 / 第3-4节 / 第5-6节 / 第7-8节 / 第9-10节 / 第11-13节 / 备注:
//   课程单元格 td 内：
//     - 多个 div.kbcontent1（display:none，历史周次备份，无教师，跳过）
//     - 一个可见 div.kbcontent（class 可能带 sykb2）：
//         **同一 div 内多门课用 "----------------------" 文本分隔线隔开**（通用版按 div 解析会丢课）
//         每门课字段顺序：
//           裸文本节点 → 课程名
//           <font title="老师">欧增球</font>
//           <font title="周次(节次)">1-3,6-9(周)[01-02节]</font>
//           <font title="教室">9232 数字化设计实训I室</font>
//   学期下拉 #xnxq01id（多年多学期）；节次模式 #kbjcmsid；周次 #zc。
//   学期切换：POST /jsxsd/xskb/xskb_list.do?viweType=0（同套强智）。
//
// ===== 桥接契约 =====
...
(function () {
    ...
    var JW_HOST = 'jiaowu.gzpyp.edu.cn';
    var KB_URL = 'https://jiaowu.gzpyp.edu.cn/jsxsd/xskb/xskb_list.do?viweType=0';
        ...
        for (var b = 0; b < blocks.length; b++) {
            ...
            for (var i = 0; i < children.length; i++) {
                ...
                if (node2.nodeType === 3) {
                    ...
                    if (!fv) continue;
                    if (title.indexOf('老师') !== -1 || title.indexOf('教师') !== -1) teacher = teacher || fv;
            ...
            out.push({
                name: name, teacher: teacher || '未安排', position: position || '',
        ...
        for (var r = 0; r < grid.length; r++) {
            ...
            for (var c = 0; c < row.length; c++) {
                ...
                for (var k = 0; k < divs.length; k++) {
                    var div = divs[k];
                    if (/display:\s*none/i.test(div.getAttribute('style') || '')) continue;
    ...
    function findTimetableIn(doc) {
        if (!doc || !doc.getElementById) return null;
        var t = doc.getElementById('kbtable');
            ...
            .then(function () {
                if (!isJwHost()) {
                    return alert('跳转到教务系统',
                        '当前不在广州番禺职业技术学院教务系统页面。\n\n' +
                        '点击「确定」将跳转到登录页，完成账号密码与验证码登录后，\n' +
                        '再进入课表页并点「执行导入」。'
