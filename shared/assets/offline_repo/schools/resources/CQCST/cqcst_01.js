async function runImportFlow() {
    ...
    if (!table || !table.innerText.includes('星期')) {
        window.shangkeBridge.showToast("没找到课表！请确保您当前在“学期理论课表”页面。");
        return;
    }
    ...
    try {
        ...
        if (courses.length === 0) {
            window.shangkeBridge.showToast("没有抓取到数据，可能当前表格为空。");
            return;
        }

        window.shangkeBridge.showToast(`提取成功，共发现 ${courses.length} 门课程，正在保存...`);
        
        const saveResult = await window.shangkeBridgePromise.saveImportedCourses(JSON.stringify(courses));
        
        if (saveResult) {
            window.shangkeBridge.showToast("导入大功告成！");
            window.shangkeBridge.notifyTaskCompletion(); 
        }

    } catch (error) {
        console.error("解析过程中发生错误:", error);
        window.shangkeBridge.showToast("解析出错啦: " + error.message);
    }
}
