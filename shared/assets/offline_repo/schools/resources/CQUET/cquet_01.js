(async function () {
  function toast(msg) {
    try {
      if (window.shangkeBridge && typeof window.shangkeBridge.showToast === "function") {
        window.shangkeBridge.showToast(String(msg));
      }
    } catch (_) {}
  }
  ...
  async function fail(message, error) {
    ...
    try {
      if (window.shangkeBridgePromise && typeof window.shangkeBridgePromise.showAlert === "function") {
        await window.shangkeBridgePromise.showAlert("提示", detail, "确定");
      }
    } catch (_) {}
  ...
  async function main() {
    if (!window.shangkeBridgePromise || !window.shangkeBridge || typeof window.shangkeBridge.notifyTaskCompletion !== "function") {
    ...
    if (parsed.timeSlots && parsed.timeSlots.length) {
      await window.shangkeBridgePromise.savePresetTimeSlots(JSON.stringify(parsed.timeSlots));
    }

    if (parsed.config) {
      await window.shangkeBridgePromise.saveCourseConfig(JSON.stringify(parsed.config));
    }
