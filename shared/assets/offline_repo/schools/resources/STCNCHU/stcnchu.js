// 南昌航空大学科技学院(stcnchu.edu.cn) 上课适配脚本
...
async function saveAppConfig() {
    ...
    return await window.shangkeBridgePromise.saveCourseConfig(JSON.stringify(config));
}
...
async function saveAppTimeSlots(campusIndex, semesterIndex) {
    ...
    return await window.shangkeBridgePromise.savePresetTimeSlots(JSON.stringify(selectedSlots));
}
...
async function runImportFlow() {
    try {
        const confirmed = await window.shangkeBridgePromise.showAlert("提示", "请确保已成功登录教务系统。是否开始导入？", "开始");
        if (!confirmed) return;

        // 1. 获取就读校区
        const campusIndex = await window.shangkeBridgePromise.showSingleSelection("选择所在校区", JSON.stringify(["共青城校区", "上海路校区"]), -1);
        if (campusIndex === null) return;

        // 2. 获取学年
        const year = await window.shangkeBridgePromise.showPrompt("选择学年", "请输入要导入课程的起始学年（例如 2025-2026 应输入2025）:", "", "validateYearInput");
        if (!year) return;

        // 3. 获取学期并记录索引
        const semesterIndex = await window.shangkeBridgePromise.showSingleSelection("选择学期", JSON.stringify(["第一学期", "第二学期"]), -1);
        if (semesterIndex === null) return;

        const semesterId = `${year}-${parseInt(year) + 1}-${semesterIndex + 1}`;

        window.shangkeBridge.showToast("正在请求数据...");
        ...
        if (finalCourses.length === 0) {
            window.shangkeBridge.showToast("未发现课程，请检查学期选择或登录状态。");
            return;
        }
        ...
        // 保存课程
        await window.shangkeBridgePromise.saveImportedCourses(JSON.stringify(finalCourses));
        
        window.shangkeBridge.showToast(`成功导入 ${finalCourses.length} 门课程`);
        window.shangkeBridge.notifyTaskCompletion();
    } catch (error) {
        window.shangkeBridge.showToast("异常: " + error.message);
    }
}
