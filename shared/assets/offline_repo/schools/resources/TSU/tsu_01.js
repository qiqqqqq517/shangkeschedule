...
async function runImportFlow() {
    try {
        window.shangkeBridge.showToast("泰山学院引擎启动，抓取数据中...");
        const courses = await fetchAndParseCourses();
        if (!courses || courses.length === 0) {
            window.shangkeBridge.showToast("解析完成，但当前课表为空");
        ...
        await window.shangkeBridgePromise.savePresetTimeSlots(JSON.stringify(SECTION_TIMES));

        window.shangkeBridge.showToast(`导入大成功！合并生成 ${courses.length} 个课块`);
        window.shangkeBridge.notifyTaskCompletion();
    } catch (error) {
        window.shangkeBridge.showToast("⚠️ " + error.message);
        window.shangkeBridge.notifyTaskCompletion();
    }
}
