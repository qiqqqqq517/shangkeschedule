...
async function askImportExams() {
    const bridge = window.shangkeBridgePromise;
...
async function runImportFlow() {
    try {
        ...
        if (!semester) { window.shangkeBridge.showToast("导入已取消"); return; }

        const { slots, map: slotMap } = parseBusinessHoursFromHtml(pageHtml);
        window.shangkeBridge.showToast(`正在获取 ${semester.label} 的课表...`);
        ...
        if (await askImportExams()) {
            ...
            window.shangkeBridge.showToast("正在获取考试安排...");
            exams.push(...parseExamList(await fetchExamData(semester.value), timeSlots));
            if (exams.length === 0) window.shangkeBridge.showToast("该学期暂时没有考试安排");
        }
        ...
        try {
            ...
            window.shangkeBridge.showToast(`课程已导入，作息时间导入失败：${error.message}`);
        }

        const examTip = exams.length > 0 ? `成功导入 ${exams.length} 门考试` : "导入完成";
        window.shangkeBridge.showToast(examTip);
