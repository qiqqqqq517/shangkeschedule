...
async function checkLoginEnvironment() {
    ...
    if (currentUrl.includes(loginUrl)) {
        window.shangkeBridge.showToast("请先登录教务系统再进行导入");
        return false;
    }
    return true;
}
...
async function getYearAndSemester() {
    try {
        ...
        const selectedIndex = await window.shangkeBridgePromise.showSingleSelection("选择导入学期", JSON.stringify(names), 0);
        if (selectedIndex === null) return null;
        const [xn, xq] = list[selectedIndex].code.split('-');
        return { xn, xq };
    } catch (error) {
        window.shangkeBridge.showToast("获取列表失败");
        return null;
    }
}
...
async function importPresetTimeSlots() {
    ...
    await window.shangkeBridgePromise.savePresetTimeSlots(JSON.stringify(slots)).catch(() => {});
}
...
async function runImportFlow() {
    ...
    // 弹窗确认
    const confirmed = await window.shangkeBridgePromise.showAlert("教务导入", "建议在‘课表查询’页面进行导入以确保数据最全。", "确定");
    if (!confirmed) return;
    ...
    if (!courses || courses.length === 0) {
        window.shangkeBridge.showToast("未找到有效课程");
        return;
    }
    ...
    // 完成
    window.shangkeBridge.showToast(`导入成功：共 ${courses.length} 门课程`);
