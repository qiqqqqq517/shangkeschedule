...
async function getSelectedSemesterId() {
    ...
    const year = await window.shangkeBridgePromise.showPrompt(
...
async function runImportFlow() {
    try {
        ...
        if (!semesterId) {
            window.shangkeBridge.showToast("用户取消了学期选择");
            return;
        }

        window.shangkeBridge.showToast("正在请求教务数据...");
        ...
        if (finalCourses.length === 0) {
            window.shangkeBridge.showToast("未发现课程数据，请检查该学期是否有课或登录是否过期");
            return;
        }
        await saveAppConfig();
