...
async function saveCourses(courses) {
    ...
    await window.shangkeBridgePromise.saveCourseConfig(JSON.stringify({ semesterTotalWeeks }));  
}
...
async function runImportFlow() {
    try {
        ...
        window.shangkeBridge.showToast(`正在获取 ${term.label} 课表...`);
        const courses = await fetchCourses(term);
        if (courses.length === 0) {
            window.shangkeBridge.showToast("该学期未查询到有效课程，请检查学期或登录状态");
            return;
        }

        await saveCourses(courses);
        window.shangkeBridge.showToast(`成功导入 ${courses.length} 个课程时段，请按校历设置开学日期`);
        window.shangkeBridge.notifyTaskCompletion();
    } catch (error) {
        console.error("课表导入失败:", error);
        window.shangkeBridge.showToast(`导入失败：${error?.message ?? error}`);
    }
}
