/**
 * 湖南商务职业技术学院教务系统(hnvcc.edu.cn) 上课适配脚本
...
async function fetchTimetable(xnxqid) {
    ...
    try {
        ...
        if (!response.ok) {
            window.shangkeBridge.showToast(`网络响应错误，状态码: ${response.status}。请检查登录状态。`);
            return null;
        }

        const htmlData = await response.text();
        window.shangkeBridge.showToast("数据获取成功，开始解析 HTML...");
        ...
        window.shangkeBridge.showToast(`请求过程中发生错误: ${error.message}`);
        return null;
    }
}
...
function parseTimetable(doc) {
    ...
    if (!timetable) {
        window.shangkeBridge.showToast("HTML 中未找到课表表格 #timetable。");
        return null;
    }
...
async function runImportFlow() {
    ...
    const academicYear = await window.shangkeBridgePromise.showPrompt(
        "选择学年", 
    ...
    window.shangkeBridge.showToast(`准备获取 ${academicYear} 学年第 ${semesterNumber} 学期数据...`);
    
    // 获取用户输入：作息季节
    const seasons = ["夏季作息", "冬季作息"];
    const seasonIndex = await window.shangkeBridgePromise.showSingleSelection(
        "选择作息季节", 
    ...
    window.shangkeBridge.showToast(`已选择：${seasonText}。`);
    ...
    if (!parsedData || parsedData.courses.length === 0) {
        window.shangkeBridge.showToast("课表解析失败或未解析到课程。请检查登录状态、学期选择或课表数据是否为空。");
        return;
    }
    ...
    try {
        ...
        window.shangkeBridge.showToast(`课程导入成功！原始 ${originalCourseCount} 门，合并 ${mergedCount} 门，最终导入 ${courses.length} 门。`);
    } catch (error) {
        window.shangkeBridge.showToast(`课程数据保存失败: ${error.message}`);
        return;
    }
    ...
    try {
        ...
        window.shangkeBridge.showToast(`课表配置更新成功！总周数：${config.semesterTotalWeeks}周。`);
    } catch (error) {
    ...
    try {
        ...
        window.shangkeBridge.showToast(`预设时间段导入成功！已使用${seasonText}。请在设置中校对具体时间。`);
    } catch (error) {
