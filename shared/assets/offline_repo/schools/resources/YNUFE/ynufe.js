...
async function importYnufeCourseSchedule() {
    ...
    try {
        ...
        if (semesters && semesters.length > 0) {
            ...
            while (true) {
                selectedIdx = await window.shangkeBridgePromise.showSingleSelection(
                    "选择学期", 
                ...
                window.shangkeBridge.showToast("必须选择一个学期才能继续导入！");
            }
            
            // 获取对应学期的课表HTML
            window.shangkeBridge.showToast(`正在获取 [${semesters[selectedIdx]}] 的课表...`);
            targetHtml = await fetchScheduleForSemester(semesterValues[selectedIdx]);

            const campuses = ["龙泉校区（默认）", "安宁校区"];
            selectedCampusIdx = await window.shangkeBridgePromise.showSingleSelection(
                "选择校区",
        ...
        if (!courses || courses.length === 0) {
            ...
            if (targetHtml && targetHtml.includes('kbtable')) {
                // 找到了课表元素但没有课程,是真的空课表
                console.log('检测到空课表');
                window.shangkeBridge.showToast('当前课表为空');
                courses = []; // 返回空数组
            } else {
                // 找不到课表元素,解析失败
                window.shangkeBridge.showToast('获取课表失败,请检查网络和页面状态');
        ...
        if (!saveResult) {
            window.shangkeBridge.showToast('保存课程失败');
        ...
        if (!timeSlotResult) {
            window.shangkeBridge.showToast('导入时间段配置失败');
        ...
        if (courses.length > 0) {
            ...
            window.shangkeBridge.showToast(`成功导入 ${courses.length} 门课程！（${campusName}）`);
        }
        console.log('课程导入完成');
        return true;
