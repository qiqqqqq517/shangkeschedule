...
async function getSelectedSemester(tagId) {
    ...
    const idx = await window.shangkeBridgePromise.showSingleSelection("选择学期", JSON.stringify(list.map(s => s.name)), 0);
...
async function applyTimeSlots() {
    ...
    return await window.shangkeBridgePromise.savePresetTimeSlots(JSON.stringify(slots));
}
