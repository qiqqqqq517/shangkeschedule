...
async function runImportFlow() {
    const isApp = typeof window.shangkeBridgePromise !== 'undefined';
    const hasToast = typeof window.shangkeBridge !== 'undefined';

    try {
        if (hasToast) {
            window.shangkeBridge.showToast("正在拉取课表，请稍候...");
        } else {
        ...
        if (courses.length === 0) {
            ...
            if (isApp) {
                await window.shangkeBridgePromise.showAlert("提示", msg, "好的");
            } else {
                alert(msg);
            }
            return;
        ...
        const slotsSaved = await window.shangkeBridgePromise.savePresetTimeSlots(JSON.stringify(timeSlots));
        if (!configSaved || !slotsSaved) {
            // 时间配置保存失败不强制中断，继续尝试导入课程
            console.warn("[HUSE] 课表时间配置保存失败，将继续尝试导入课程。");
            window.shangkeBridge.showToast("时间配置保存失败，继续导入课程...");
        }

        // APP 环境：保存课程数据
        const courseSaved = await window.shangkeBridgePromise.saveImportedCourses(JSON.stringify(courses));
        if (!courseSaved) {
            console.error("[HUSE] 课程数据保存失败。");
            window.shangkeBridge.showToast("课程保存失败，请重试！");
            return;
        }

        console.log(`[HUSE] 导入完成，共写入 ${courses.length} 门课程。`);
        window.shangkeBridge.showToast(`成功导入 ${courses.length} 门课程及作息时间！`);
