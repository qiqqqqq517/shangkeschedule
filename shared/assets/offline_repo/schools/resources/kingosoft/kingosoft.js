// 青果教务系统通用适配器（DOM 矩阵解析版）
// 自动识别课表表格：星期表头 + 节次行，支持 rowspan/colspan 与 iframe 内课表。
(function () {
    'use strict';

    var PLATFORM_NAME = '青果教务系统';
    var PREFERRED_SELECTORS = ['.courseTable','#kbtable','table[class*=course]','table[class*=kb]'];
    var BODY_KEYWORDS = ['青果','学生个人课表'];
    var URL_PATTERNS = [
        /jwc/g,
        /jwxt/g,
        /kingosoft/g,
    ...
    async function runImport() {
        try {
            ...
            if (!found.table) {
                showToast('未找到课表表格，请进入青果教务系统的学生个人课表页面并等待表格加载后再试');
                return;
            }
    ...
    if (matchesPlatformHint()) {
        showToast('检测到青果教务系统，点击导入按钮抓取课表');
    }
})();
