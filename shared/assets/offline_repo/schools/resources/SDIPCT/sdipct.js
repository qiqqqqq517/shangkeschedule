...
async function fetchCourses(academicYear, semesterIndex) {
    ...
    try {
        ...
        window.shangkeBridge.showToast("网络请求失败，请检查登录状态");
        return null;
    }
}
...
async function runImportFlow() {
    ...
    if (!courses || courses.length === 0) {
        if (courses && courses.length === 0) window.shangkeBridge.showToast("该学期暂无课程数据");
        return;
    }
    ...
    try {
        ...
        window.shangkeBridge.showToast("保存失败: " + e.message);
    }
}
