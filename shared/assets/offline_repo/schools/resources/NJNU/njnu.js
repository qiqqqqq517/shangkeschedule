...
async function fetchSemesterStartDate(academicYear, semesterCode) {
    try {
        ...
        window.shangkeBridge.showToast("获取错误，需要手动设置开学日期");
        return null;
    }
}
/**
...
function applyCourseChanges(parsedCourses, rawChanges) {
    ...
    if (successCount > 0) {
        window.shangkeBridge.showToast(`已应用 ${successCount} 条调课/停课变更，获得实际课表。`);
    }
...
async function getAcademicYear() {
    ...
    const yearSelection = await window.shangkeBridgePromise.showPrompt(
        "选择学年",
...
async function fetchAndParseCourses(academicYear, semesterCode) {
    ...
    try {
        ...
        window.shangkeBridge.showToast("请求课表 API 失败，请检查网络和登录状态,以及是否跳转到课表页面");
    ...
    if (rawCourses.length === 0) {
        window.shangkeBridge.showToast("该学期未查询到您的课程数据。");
        return null;
    }
    ...
    try {
        ...
        window.shangkeBridge.showToast("请求调课 API 失败，将使用未调整的课表数据。");
...
async function saveCourses(parsedCourses) {
    if (parsedCourses.length === 0) {
        window.shangkeBridge.showToast("没有有效的课程数据可供保存。");
        return true;
    }
    try {
        await window.shangkeBridgePromise.saveImportedCourses(JSON.stringify(parsedCourses));
        window.shangkeBridge.showToast(`成功导入 ${parsedCourses.length} 门课程！`);
        return true;
    } catch (error) {
        window.shangkeBridge.showToast(`保存课程数据失败: ${error.message}`);
        return false;
    }
}
...
async function importPresetTimeSlots() {
    window.shangkeBridge.showToast("正在导入预设节次时间...");
    ...
    try {
        await window.shangkeBridgePromise.savePresetTimeSlots(JSON.stringify(presetTimeSlots));
...
async function saveConfig(configData) {
    try {
        await window.shangkeBridgePromise.saveCourseConfig(JSON.stringify(configData));
        window.shangkeBridge.showToast("课表配置更新成功！");
        return true;
    } catch (error) {
        window.shangkeBridge.showToast("保存配置失败: " + error.message);
        return false;
    }
}
...
async function runImportFlow() {
    window.shangkeBridge.showToast("南京师范大学课程导入流程启动...");
    ...
    // 7. 流程完全成功，发送结束信号。
    window.shangkeBridge.showToast("所有任务已完成！课表导入成功。");
