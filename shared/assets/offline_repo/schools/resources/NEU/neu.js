...
async function showCampusSelection() {
    ...
    try {
        const idx = await window.shangkeBridgePromise.showSingleSelection("选择你所在的校区", JSON.stringify(campuses), 2);
        return idx !== -1 ? campuses[idx] : false;
    } catch(e) {
        window.shangkeBridge.showToast("显示校区列表出错：" + e.message);
        return false;
    }
}
...
async function SaveCourses(lessons) {
    await window.shangkeBridgePromise.saveImportedCourses(JSON.stringify(lessons));
}
...
async function importTimeSlotsByCampus(campus) {
    ...
    await window.shangkeBridgePromise.savePresetTimeSlots(JSON.stringify(slots));
}
...
async function SaveConfig() {
    ...
    await window.shangkeBridgePromise.saveCourseConfig(JSON.stringify(cfg));
}
...
async function runAllDemosSequentially() {
    window.shangkeBridge.showToast("开始导入课表...");
    const campus = await showCampusSelection();
    if (!campus) { window.shangkeBridge.showToast("已取消导入"); return; }
    const semester = await showSemesterSelection();
    if (!semester) { window.shangkeBridge.showToast("已取消导入"); return; }
    
    window.shangkeBridge.showToast("正在获取课表数据...");
    let lessons;
    try {
        lessons = await fetchCoursesFromAPI(semester);
        if (!lessons.length) { window.shangkeBridge.showToast("未获取到任何课程"); return; }
    ...
    await SaveConfig();
    window.shangkeBridge.showToast("课表导入完成！");
    
    const importExams = await askImportExams();
    if (importExams) {
        window.shangkeBridge.showToast("正在获取考试数据...");
        try {
            const examLessons = await fetchExamsFromAPI(semester);
            if (examLessons.length === 0) {
                window.shangkeBridge.showToast("未获取到考试数据");
            } else {
                const allLessons = [...lessons, ...examLessons];
                await SaveCourses(allLessons);
                window.shangkeBridge.showToast(`已导入 ${examLessons.length} 条考试记录（合并至课表）`);
            }
        } catch(e) {
            window.shangkeBridge.showToast("导入考试失败: "+e.message);
            console.error(e);
        }
    }
