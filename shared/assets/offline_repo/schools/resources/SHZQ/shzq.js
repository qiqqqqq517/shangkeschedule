...
async function getYearAndSemester() {
    try {
        ...
        if (yearSelection === null) {
            window.shangkeBridge.showToast("导入取消：未选择学年。");
            return null;
        }
        ...
        if (semesterIndex === null || semesterIndex === -1) {
            window.shangkeBridge.showToast("导入取消：未选择学期。");
            return null;
        }
        ...
        window.shangkeBridge.showToast("获取学年学期失败：" + error.message);
        return null;
    }
}
...
async function fetchCourses(xnm, xqm) {
    try {
        window.shangkeBridge.showToast(`正在获取学期课程...`);
        ...
        if (courses.length === 0) {
            window.shangkeBridge.showToast("未找到任何课程数据，请检查学年学期或登录状态。");
            return null;
        }
        return courses;
    } catch (error) {
        console.error("JS: 获取课程数据时出错:", error);
        window.shangkeBridge.showToast(`获取课程失败: ${error.message || error}`);
        return null;
    }
}
...
async function saveCourses(courses) {
    try {
        ...
        window.shangkeBridge.showToast(`保存失败: ${error.message || error}`);
        return false;
    }
}
