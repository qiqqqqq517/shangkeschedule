...
async function saveCourses(courses) {
    try {
        ...
        await window.shangkeBridgePromise.showAlert("保存失败", `课程保存失败：${error.message}`, "确定");
        return false;
    }
}
...
async function saveOptionalSettings(config) {
    ...
    try {
        ...
        window.shangkeBridge.showToast(`作息时间保存失败：${error.message}`);
    }
}
...
async function runImportFlow() {
    ...
    try {
        ...
        await window.shangkeBridgePromise.showAlert("导入失败", error.message, "确定");
        return;
    }
    ...
    window.shangkeBridge.showToast(`课程导入成功，共导入 ${result.courses.length} 条课程安排。`);
