...
async function fetchAndParseJwData() {
    try {
        window.shangkeBridge.showToast("正在解析网页课表...");
...
async function runImportFlow() {
    const alertResult = await window.shangkeBridgePromise.showAlert(
        "教务网页课表导入",
    ...
    if (await saveToApp(result)) {
        window.shangkeBridge.showToast(`成功从网页导入 ${result.courses.length} 个课程时段`);
