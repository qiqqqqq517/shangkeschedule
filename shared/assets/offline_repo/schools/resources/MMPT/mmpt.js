...
async function selectCampusSchedule() {
    ...
    const campusIndex = await window.shangkeBridgePromise.showSingleSelection(
        "选择校区时间表", 
    ...
    if (campusIndex === null) {
        window.shangkeBridge.showToast("未选择校区，使用南校区时间表");
...
async function selectAcademicYear() {
    ...
    const yearIndex = await window.shangkeBridgePromise.showSingleSelection(
        "选择学年", 
...
async function fetchCourseData() {
    try {
        ...
        window.shangkeBridge.showToast("获取课程数据失败：" + err.message);
        return null;
    }
}
...
async function saveCourses(courses) {
    try {
        ...
        if (result === true) {
            window.shangkeBridge.showToast("课程导入成功！");
            return true;
        } else {
            window.shangkeBridge.showToast("课程导入失败，请查看日志！");
            return false;
        }
    } catch (err) {
        console.error("保存课程失败:", err);
        window.shangkeBridge.showToast("保存课程失败：" + err.message);
        return false;
    }
}
...
async function importPresetTimeSlots() {
    try {
        ...
        if (result === true) {
            window.shangkeBridge.showToast(`时间段导入成功！共导入${timeSlots.length}个时间段`);
        } else {
            window.shangkeBridge.showToast("时间段导入失败，请查看日志！");
        }
    } catch (err) {
        console.error("时间段导入失败:", err);
        window.shangkeBridge.showToast("时间段导入失败：" + err.message);
    }
}
...
async function runImportFlow() {
    // 检查用户是否已登录
    if (!isUserLoggedIn()) {
        window.shangkeBridge.showToast("检测到未登录状态，请先登录后再使用课程导入功能！");
        return;
    }
    ...
    try {
        // 1️⃣ 获取课程数据（选择学年学期）
        window.shangkeBridge.showToast("请选择学年和学期...");
        
        // 从新系统获取课程数据
        const responseData = await fetchCourseData();
        if (!responseData) {
            window.shangkeBridge.showToast("获取课程数据失败！");
            return;
        }
        
        // 2️⃣ 解析课程数据
        window.shangkeBridge.showToast("正在解析课程数据...");
        ...
        // 3️⃣ 合并重复课程
        window.shangkeBridge.showToast("正在合并重复课程...");
        const mergedCourses = mergeDuplicateCourses(courses);
        
        // 4️⃣ 保存课程
        window.shangkeBridge.showToast("正在保存课程数据...");
        ...
        // 5️⃣ 导入预设时间段（用户选择校区）
        window.shangkeBridge.showToast("请选择校区时间表...");
        await importPresetTimeSlots();

        // ✅ 只有所有步骤都成功完成，才通知任务完成
        window.shangkeBridge.showToast(`课程导入成功，共导入 ${mergedCourses.length} 门课程！`);
        console.log("JS：整个导入流程执行完毕并成功。");
        window.shangkeBridge.notifyTaskCompletion();
        
    } catch (err) {
        console.error("导入流程失败:", err);
        window.shangkeBridge.showToast("导入失败：" + err.message);
