...
function applyCustomTimeLogic(courses) {
    ...
    // 辅助函数：根据节次从方案中提取时间
    const getTimeFromScheme = (scheme, section) => {
        const slot = scheme.find(s => s.number === section);
    ...
    return courses.map(course => {
        ...
        if (shouldApplyCustom) {
            ...
            if (isWeekend) {
                ...
                // 辅助函数：如果是 3 或 4 节则取特殊方案，否则取全局标准时间
...
function parseJsonData(jsonData) {
    ...
    // 课程时间特殊处理
    const processedCourses = applyCustomTimeLogic(finalCourseList);
