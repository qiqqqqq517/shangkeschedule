...
function parseJsonData(jsonData) {
    ...
    window.shangkeBridge.showToast(`去重前：${finalCourseList.length} 门，去重后：${merged.length} 门`);
    return merged;
}
/**
 * 根据课程周次数组推断学期总周数。
...
async function promptUserToExit() {
    ...
    const rawIndex = await window.shangkeBridgePromise.showSingleSelection(
        "导入流程已结束",
...
async function getSemesterStartDate(academicYear, semesterIndex, weekList = null) {
    ...
    if (!Array.isArray(data) || data.length === 0) {
        ...
        try {
            ...
            window.shangkeBridge.showToast("未能获取学期周次数据，开始日期可能需要手动填写。");
    ...
    const input = await window.shangkeBridgePromise.showPrompt(
        "学期开始日期喵~",
...
async function runImportFlow() {
    ...
    try {
        ...
        try {
            ...
            window.shangkeBridge.showToast("获取周次数据失败，将使用课程周数或手动输入。");
            schoolWeekList = [];
        }
        ...
        try {
            ...
            window.shangkeBridge.showToast(error.message);
        ...
        if (weekSource === null) {
            window.shangkeBridge.showToast("未选择周数来源，导入已取消。");
        ...
        if (weekSource === 0) {
            ...
            if (schoolMaxWeeks) {
                ...
                window.shangkeBridge.showToast("教务系统周数不可用，已使用课程最大周数。");
                finalWeeks = courseMaxWeeks;
            }
        } else {
        ...
        if (shouldImportTimeSlots) {
            ...
            window.shangkeBridge.showToast("已跳过导入时间段。");
        }
