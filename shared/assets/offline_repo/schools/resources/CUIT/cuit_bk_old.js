...
(function() {
    ...
    async function runImportFlow() {
        if (!window.shangkeBridgePromise) throw new Error("AndroidBridgePromise 不可用");
        window.shangkeBridge.showToast("正在探测教务参数...");
        ...
        if (!params.studentId || !params.tagId) {
            await window.shangkeBridgePromise.showAlert("参数探测失败", "未能识别学生ID或学期组件", "确定");
            return;
        }
        ...
        const selectIndex = await window.shangkeBridgePromise.showSingleSelection(
            "请选择导入学期",
        ...
        if (courses.length === 0) {
            await window.shangkeBridgePromise.showAlert("解析失败", "未提取到课程数据", "确定");
            return;
        }
        ...
        await window.shangkeBridgePromise.savePresetTimeSlots(JSON.stringify(getPresetTimeSlots()));
