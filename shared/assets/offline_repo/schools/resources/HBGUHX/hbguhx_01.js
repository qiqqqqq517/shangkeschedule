// 河北地质大学华信学院强智教务 (61.182.88.214:8090) 上课适配脚本
...
async function saveCourseDataToApp(courses, timeSlots, semesterId) {
    ...
    // 保存学期配置
    await window.shangkeBridgePromise.saveCourseConfig(JSON.stringify({
    ...
    // 保存课程数据
    return await window.shangkeBridgePromise.saveImportedCourses(JSON.stringify(courses));
}
    ...
    try {
        ...
        if (finalCourses.length === 0) {
            ...
            if (initialCourses.length > 0) {
                ...
                window.shangkeBridge.showToast(`成功导入 ${initialCourses.length} 门课程`);
        ...
        window.shangkeBridge.showToast(`成功导入 ${finalCourses.length} 门课程（当前第 ${calculateCurrentWeek(semesterId)} 周）`);
        window.shangkeBridge.notifyTaskCompletion();
