...
// 主流程
(async function () {
    if (!(window.shangkeBridgePromise && window.shangkeBridgePromise.saveImportedCourses)) return;
    ...
    await window.shangkeBridgePromise.savePresetTimeSlots(JSON.stringify(PRESET_TIME_SLOTS));
    await window.shangkeBridgePromise.saveCourseConfig(JSON.stringify({ semesterStartDate: null, semesterTotalWeeks: totalWeeks || 20, firstDayOfWeek: 1 }));
    await window.shangkeBridgePromise.saveImportedCourses(JSON.stringify(courses));
    try { window.shangkeBridge.notifyTaskCompletion(); } catch (e) { }
})();