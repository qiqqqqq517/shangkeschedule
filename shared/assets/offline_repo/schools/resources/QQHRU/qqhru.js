...
async function fetchAndParseJwData(academicYear, semesterIndex) {
    try {
        window.shangkeBridge.showToast("正在获取教务初始化凭证...");
        ...
        window.shangkeBridge.showToast("同步失败: " + e.message);
        return null;
    }
}
...
async function saveToApp(result) {
    const courseSuccess = await window.shangkeBridgePromise.saveImportedCourses(JSON.stringify(result.courses));
    if (!courseSuccess) return false;

    if (result.timeSlots && result.timeSlots.length > 0) {
        await window.shangkeBridgePromise.savePresetTimeSlots(JSON.stringify(result.timeSlots));
    }
...
async function runImportFlow() {
    ...
    if (await saveToApp(result)) {
        window.shangkeBridge.showToast(`成功导入 ${result.courses.length} 个课程时段`);
