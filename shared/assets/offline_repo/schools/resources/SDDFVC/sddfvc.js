// 山东药品食品职业学院(sddfvc.edu.cn) 上课适配脚本
...
async function getSelectedSemesterId(apiToken) {
    ...
    const selectedIdx = await window.shangkeBridgePromise.showSingleSelection(
        "请确认导入学期",
...
async function fetchFullSemesterData(apiToken, semesterId) {
    ...
    for (let w = 1; w <= totalWeeks; w++) {
        window.shangkeBridge.showToast(`正在获取第 ${w}/${totalWeeks} 周...`);
        try {
...
async function runImportFlow() {
    try {
        ...
        // 直接进入周遍历模式
        window.shangkeBridge.showToast("开始抓取周课表数据...");
        const finalCourses = await fetchFullSemesterData(apiToken, semesterId);

        if (finalCourses.length === 0) {
            window.shangkeBridge.showToast("未发现任何课程数据");
            return;
        }
