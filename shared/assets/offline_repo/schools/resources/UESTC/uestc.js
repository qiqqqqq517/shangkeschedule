...
async function runImportFlow() {
  ...
  try {
    // ── Step 1: 欢迎提示 ──
    const ready = await window.shangkeBridgePromise.showAlert(
      '电子科技大学 - 教务导入',
    ...
    if (activities.length === 0) {
      ...
      await window.shangkeBridgePromise.showAlert('导入结果', '未在当前学期找到课程数据。', '知道了');
      return;
    }
    ...
    try {
      await window.shangkeBridgePromise.savePresetTimeSlots(JSON.stringify(DEFAULT_TIME_SLOTS));
      console.log('✅ 时间段已保存');
