// 南软教务系统通用适配器（DOM 矩阵解析版）
// 自动识别课表表格：星期表头 + 节次行，支持 rowspan/colspan 与 iframe 内课表。
(function () {
    'use strict';

    var PLATFORM_NAME = '南软教务系统';
    var PREFERRED_SELECTORS = ['.course-timetable','#timetable','table[class*=time]','table[class*=kb]'];
    var BODY_KEYWORDS = ['南软','研究生课表'];
    var URL_PATTERNS = [
        /jw/g,
        /yjs/g,
        /graduate/g,
    ...
    async function runImport() {
        try {
            ...
            if (!found.table) {
                showToast('未找到课表表格，请进入南软教务系统的学生个人课表页面并等待表格加载后再试');
                return;
            }
    ...
    if (matchesPlatformHint()) {
        showToast('检测到南软教务系统，点击导入按钮抓取课表');
    }
})();
