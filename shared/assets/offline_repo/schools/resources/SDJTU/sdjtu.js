...
async function getSemesterStartDate(semesterId) {
    ...
    const dateInput = await window.shangkeBridgePromise.showPrompt(
        "设置开学日期",
...
async function runImportFlow() {
    try {
        ...
        if (!doc) {
            window.shangkeBridge.showToast("未找到课表页面，请先打开【学期理论课表】");
            return;
        }
        ...
        if (startDate === null) {
            window.shangkeBridge.showToast("已取消开学日期输入");
            return;
        }
        ...
        if (courses.length === 0) {
            window.shangkeBridge.showToast("未获取到课程数据，该学期可能暂无课表");
            return;
        }
        ...
        window.shangkeBridge.showToast(`[${semesterId}] 成功导入 ${courses.length} 条课程记录！`);
