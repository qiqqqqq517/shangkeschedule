...
async function fetchCampusList() {
    ...
    try {
        ...
        window.shangkeBridge.showToast(`获取校区列表失败: ${error.message}`);
...
async function selectCampus(defaultXqdm) {
    ...
    if (!campusList) {
        ...
        window.shangkeBridge.showToast("获取校区列表失败，使用默认校区继续导入。");
        return {
            xqdm: defaultXqdm,
            xqmc: ""
        };
...
async function fetchTimeAndWeekData(xnxq, xqdm) {
    ...
    window.shangkeBridge.showToast("正在获取课表配置信息...");
    ...
    try {
        ...
        window.shangkeBridge.showToast(`获取配置信息失败，将继续导入课程: ${error.message}`);
...
async function fetchCourseData(xnxq, xhid, xqdm) {
    ...
    window.shangkeBridge.showToast(`正在获取 ${xnxq} 的课程数据...`);
    ...
    try {
        ...
        if (courses.length === 0) {
            window.shangkeBridge.showToast("未找到任何课程数据，本学期可能无课。");
            return null;
        }
        ...
        window.shangkeBridge.showToast(`获取课程数据失败: ${error.message}`);
...
async function saveCourses(courses) {
    ...
    window.shangkeBridge.showToast(`正在保存 ${courses.length} 门课程...`);
...
async function importPresetTimeSlots(timeSlots) {
    ...
    window.shangkeBridge.showToast(`正在导入作息时间...`);
...
async function saveCourseConfig(semesterStartDate) {
    ...
    try {
        ...
        window.shangkeBridge.showToast("保存课表配置失败: " + error.message);
...
async function runImportFlow() {
    ...
    if (!params) {
        window.shangkeBridge.showToast("无法从页面获取必要参数，请确保在正确的页面执行脚本。");
        return;
    }
    ...
    if (xnxq === null) {
        window.shangkeBridge.showToast("导入已取消，未选择学年学期。");
        return;
    }
    ...
    if (!selectedCampus) {
        window.shangkeBridge.showToast("导入已取消，未选择校区。");
        return;
    }
    ...
    // 11. 完成
    window.shangkeBridge.showToast(`导入成功！共导入 ${courses.length} 门课程。`);
