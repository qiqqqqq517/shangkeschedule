...
function applyCourseChanges(parsedCourses, rawChanges) {
    ...
    if (successCount > 0) {
        window.shangkeBridge.showToast(`已应用 ${successCount} 条调课/停课变更。`);
    }
...
async function selectSemester() {
    ...
    try {
        ...
        window.shangkeBridge.showToast("获取学期列表失败，请检查登录状态。");
        return null;
    }

    if (semesterList.length === 0) {
        window.shangkeBridge.showToast("未查询到学期数据。");
        return null;
    }
...
async function importPresetTimeSlots() {
    window.shangkeBridge.showToast("正在获取作息时间...");
    const headers = {
    ...
    if (!presetTimeSlots || presetTimeSlots.length === 0) {
        window.shangkeBridge.showToast("未能获取线上作息时间，已启用备用作息表。");
    ...
    try {
        ...
        window.shangkeBridge.showToast("保存时间段失败: " + error.message);
        return false;
    }
}
...
async function fetchAndParseCourses(semesterObj) {
    ...
    try {
        ...
        window.shangkeBridge.showToast("请求课表 API 失败，请检查网络或登录状态。");
    ...
    try {
        ...
        window.shangkeBridge.showToast("请求调课 API 失败，将使用原始课表。");
...
async function runImportFlow() {
    window.shangkeBridge.showToast("课程导入流程启动...");
