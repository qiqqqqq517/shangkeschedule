...
async function scrapeAndParseCourses() {
    window.shangkeBridge.showToast("正在检查页面并抓取课程数据...");
    ...
    try {
        ...
        if (!text.includes("课表查询")) {
            await window.shangkeBridgePromise.showAlert("导入失败", `当前页面似乎不是学生课表查询页面。请检查：\n${tips}`, "确定");
            return null;
        }

        const typeElement = document.querySelector("#shcPDF");
        if (!typeElement) {
            await window.shangkeBridgePromise.showAlert("导入失败", "未能识别课表视图类型，请确认您已点击查询且课表已加载完毕。", "确定");
            return null;
        }
        ...
        if (!tableElement) {
            await window.shangkeBridgePromise.showAlert("导入失败", `未能找到课表主体（${type} 视图），请确认您已点击查询且课表已加载完毕。`, "确定");
            return null;
        }

        const courses = type === "list" ? parseList() : parseTable();
        if (!courses.length) {
            window.shangkeBridge.showToast("未找到任何课程数据，请检查学年学期是否正确或本学期无课。");
            return null;
        }
        ...
        window.shangkeBridge.showToast(`抓取或解析失败: ${error.message}`);
        console.error("JS: Scrape/Parse Error:", error);
        await window.shangkeBridgePromise.showAlert("抓取或解析失败", `发生错误：${error.message}`, "确定");
        return null;
    }
}
...
async function importPresetTimeSlots() {
    try {
        await window.shangkeBridgePromise.savePresetTimeSlots(JSON.stringify(TIME_SLOTS));
...
async function runImportFlow() {
    const alertConfirmed = await window.shangkeBridgePromise.showAlert(
        "徐州工程学院课表导入",
    ...
    if (typeof window.jQuery === "undefined" && typeof $ === "undefined") {
        ...
        window.shangkeBridge.showToast(errorMsg);
        await window.shangkeBridgePromise.showAlert("导入失败", `${errorMsg}\n请尝试刷新页面后重试。`, "确定");
        return;
    }
    ...
    window.shangkeBridge.showToast(`课程导入成功，共导入 ${courses.length} 门课程！`);
