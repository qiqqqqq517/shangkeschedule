...
const selectYearTerm = async (weekData) => {
    ...
    if (selectedIndex === null || Number(selectedIndex) === -1) {
        window.shangkeBridge.showToast('已取消课程导入');
        return null;
    }
...
const saveSchedule = async (parsedSchedule) => {
    await saveBridgeData(
        '课表配置',
        () => window.shangkeBridgePromise.saveCourseConfig(JSON.stringify(parsedSchedule.courseConfig)),
    );
    await saveBridgeData(
        '节次时间',
        () => window.shangkeBridgePromise.savePresetTimeSlots(JSON.stringify(parsedSchedule.timeSlots)),
    );
    await saveBridgeData(
        '课程数据',
        () => window.shangkeBridgePromise.saveImportedCourses(JSON.stringify(parsedSchedule.courses)),
    );
};
...
const runImportFlow = async () => {
    ...
    if (courses.length === 0) {
        window.shangkeBridge.showToast(`该学期（${formatYearTerm(yearTerm)}）暂无可导入的课程，请确认所选学期后重试。`);
        return;
    }
    ...
    window.shangkeBridge.showToast(`已成功导入${formatYearTerm(yearTerm)}课表。`);
...
const runImportFlowSafely = async () => {
    try {
        ...
        window.shangkeBridge.showToast(`导入未完成：${userMessage}`);
    }
};
