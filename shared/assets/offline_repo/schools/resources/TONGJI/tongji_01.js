(() => {
  ...
  async function determineTerm() {
    ...
    const useOtherTerm = await window.shangkeBridgePromise.showSingleSelection(
      "是否使用当前学期",
      JSON.stringify([
    ...
    if (useOtherTerm === null)
      window.shangkeBridge.showToast("未选择学期，使用当前学期");
    else if (useOtherTerm === 1) {
      window.shangkeBridge.showToast("正在加载学期列表");
      ...
      else window.shangkeBridge.showToast("未选择学期，使用当前学期");
    }
  ...
  async function main() {
    ...
    const result = await Promise.allSettled([
      window.shangkeBridgePromise.saveImportedCourses(JSON.stringify(courseInfo)),
      window.shangkeBridgePromise.savePresetTimeSlots(JSON.stringify(timeSlots)),
      window.shangkeBridgePromise.saveCourseConfig(
