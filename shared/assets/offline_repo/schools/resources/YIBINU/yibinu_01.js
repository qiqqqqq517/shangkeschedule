...
async function fetchExperimentCourses(studentId,yearterm,authorization) {
    try {
        ...
        if (!authorization) {
            ...
            window.shangkeBridge.showToast("未找到登录凭证，请重新登录");
            return [];
        }
        ...
        if (!res.ok) {
            ...
            window.shangkeBridge.showToast(`实验课错误：${errorData.msg || res.status}`);
            return {};
        }
        ...
        console.error(e);
        window.shangkeBridge.showToast("实验课获取失败: " + e.message);
        return {};
    }
}
...
function parseExperimentCoursesData(jsonData) {
    ...
    if (!jsonData || jsonData.code !== 200 || !jsonData.result?.list) {
        window.shangkeBridge.showToast("未获取到实验课数据！");
        return [];
    }
...
async function establishSession() {
    try {
        ...
        console.error("错误：", e);
        window.shangkeBridge.showToast("会话连接建立失败，请重试！");
    }
}
...
async function fetchTheoryCourses(yearTerm, studentId) {
    try {
        ...
        console.error("错误：", e);
        window.shangkeBridge.showToast("获取课表失败：" + e.message);
        return [];
    }
}
...
async function fetchTermInfo(academicYear,term) {
    try {
        ...
        console.error("错误：", e);
        window.shangkeBridge.showToast("学期信息获取失败" );
        return {};
    }
}
...
function parseYearTermJson(jsonData){
    ...
    if(jsonData.datas?.cxjcs?.rows.length===0){
        window.shangkeBridge.showToast("所查询的学期信息为空");
        return {};
    }
...
function parseTheoryJson(jsonData) {
    ...
    if(jsonData.datas?.xskcb?.extParams?.code!==1){
        window.shangkeBridge.showToast(jsonData.datas?.xskcb?.extParams?.msg);
        return [];
    }
...
async function saveAllCourses(courses) {
    try { 
        await window.shangkeBridgePromise.saveImportedCourses(JSON.stringify(courses, null, 2)); 
        return true; 
    }
    catch (e) { 
        window.shangkeBridge.showToast("课程保存失败"); 
        return false; 
    }
}
//保存学期信息配置
...
async function importTimeSlots() {
    try { 
        await window.shangkeBridgePromise.savePresetTimeSlots(JSON.stringify(getTimeSlots())); 
...
async function askImportExperiment(theoryNum,startDate) {
    ...
    return await window.shangkeBridgePromise.showSingleSelection(
...
async function runImportFlow() {
    if (isTheoryLoginPage()) { 
        window.shangkeBridge.showToast("请先登录智慧校园！"); 
        return; 
    }

    const studentId = localStorage.getItem('ampUserId');
    if (!studentId) {
        window.shangkeBridge.showToast("请确保已登录智慧校园！");
        return;
    }
    ...
    window.shangkeBridge.showToast("稍等一下哦,信息获取中");
    ...
    window.shangkeBridge.showToast(`导入成功！共 ${theoryNum} 门理论课`);
...
async function runExpAutoMerge() {
    try {
        ...
        if (!decodeData) {
            window.shangkeBridge.showToast("未检测到理论课参数，请重新从理论课流程开始！");
            return;
        }
        ...
        if (!theoryCourses || !yearTerm || !studentId) {
            window.shangkeBridge.showToast("参数不完整，请重新导入理论课！");
            return;
        }
        ...
        window.shangkeBridge.showToast(`导入成功！理论课${theoryNum}门 + 实验课${expNum}门`);
        window.shangkeBridge.notifyTaskCompletion();
    } catch (err) {
        console.error(err);
        window.shangkeBridge.showToast("自动导入失败！请退出重新开始流程");
    }
}
