...
async function parseAndImportCourses() {
    ...
    if (!tableElement) {
        ...
        window.shangkeBridge.showToast('未找到课程表格，请确保在正确的页面！');
        return false;
    }
...
async function saveCourseConfig() {
    ...
    try {
        ...
        if (result === true) {
            ...
            window.shangkeBridge.showToast(`配置导入成功！学期: ${semesterInfo.semester}, 开学: ${courseConfigData.semesterStartDate}`);
            return true;
        } else {
            console.log("课表配置导入未成功，结果：" + result);
            window.shangkeBridge.showToast("配置导入失败，请查看日志。");
            return false;
        }
    } catch (error) {
...
async function runImportFlow() {
    ...
    window.shangkeBridge.showToast("开始解析课程表...");
    ...
    console.log("课程导入完成。");
    window.shangkeBridge.showToast("课程导入完成！");
...
if (typeof window.shangkeBridge !== 'undefined' && window.shangkeBridge) {
    runImportFlow();
}
