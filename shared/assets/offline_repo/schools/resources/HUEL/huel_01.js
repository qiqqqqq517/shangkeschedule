...
async function selectSemesterStartDate(xnm, xqm) {
  ...
  const picked = await window.shangkeBridgePromise.showPrompt(
    '选择开学日期',
    '请输入开学日期（YYYY-MM-DD）',
    defaultDate,
...
async function run() {
  try {
    ...
    if (!courses.length) {
      window.shangkeBridge.showToast('导入失败: 未获取到课表数据');
      return;
    }
    ...
    window.shangkeBridge.showToast(`导入成功：${courses.length} 门`);
