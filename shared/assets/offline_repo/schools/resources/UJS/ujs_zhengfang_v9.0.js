...
// 2026.08.23 第二版
// 桥接接口升级至 v2（window.shangkeBridge / window.shangkeBridgePromise）。
...
async function whetherSummerTimeSlot() {
    ...
    //     window.shangkeBridge.showToast("无法获取作息时间公告，智能选择回退到预设时间。");
...
async function selectSemester() {
    ...
    const semesterIndex = await window.shangkeBridgePromise.showSingleSelection(
        "选择学期",
...
async function selectTimeSlot() {
    ...
    const timeSlotIndex = await window.shangkeBridgePromise.showSingleSelection(
        "选择作息时间",
...
async function fetchAndParseCourses(academicYear, semesterIndex) {
    window.shangkeBridge.showToast("正在请求课表数据...");
    ...
    try {
        ...
        try {
            ...
            window.shangkeBridge.showToast("数据返回格式错误，可能是您未成功登录或会话已过期。");
            return null;
        }
        ...
        if (courses.length === 0) {
            window.shangkeBridge.showToast("未找到任何课程数据，请检查所选学年学期是否正确或本学期无课，或教务系统需要二次登录。");
            return null;
        }
        ...
        window.shangkeBridge.showToast(`请求或解析失败: ${error.message}`);
...
async function saveCourses(parsedCourses) {
    window.shangkeBridge.showToast(`正在保存 ${parsedCourses.length} 门课程...`);
    console.log(`JS: 尝试保存 ${parsedCourses.length} 门课程...`);
    try {
        await window.shangkeBridgePromise.saveImportedCourses(JSON.stringify(parsedCourses, null, 2));
        console.log("JS: 课程保存成功！");
        return true;
    } catch (error) {
        window.shangkeBridge.showToast(`课程保存失败: ${error.message}`);
...
async function saveCourseConfigIfPossible(courses, academicYear, semesterIndex, firstDayOfWeek) {
    ...
    if (!config) {
        window.shangkeBridge.showToast("未取到本学期开学日期，已跳过课表配置，请在应用内手动设置开学日期。");
    ...
    try {
        ...
        window.shangkeBridge.showToast(
            `课表配置更新成功！开学日期 ${config.semesterStartDate}，总周数 ${config.semesterTotalWeeks} 周。`
        );
    } catch (error) {
        window.shangkeBridge.showToast(`课表配置保存失败: ${error.message}`);
...
async function importPresetTimeSlots(timeSlots) {
    ...
    if (timeSlots.length > 0) {
        window.shangkeBridge.showToast(`正在导入 ${timeSlots.length} 个预设时间段...`);
        try {
            await window.shangkeBridgePromise.savePresetTimeSlots(JSON.stringify(timeSlots));
            window.shangkeBridge.showToast("预设时间段导入成功！");
        ...
        window.shangkeBridge.showToast("警告：时间段为空，未导入时间段信息。");
...
async function runImportFlow() {
    if (isLoginPage()) {
        window.shangkeBridge.showToast("导入失败：请先登录教务系统！");
    ...
    if (!alertConfirmed) {
        window.shangkeBridge.showToast("用户取消了导入。");
    ...
    window.shangkeBridge.showToast(`课程导入成功，共导入 ${coursesWithCustomTime.length} 门课程！`);
