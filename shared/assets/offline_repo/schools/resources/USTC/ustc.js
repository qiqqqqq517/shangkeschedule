...
async function runImportFlow() {
    ...
    window.shangkeBridge.showToast("正在等待课表加载...");
    var timetableDoc = await pollUntilTableReady();
    if (!timetableDoc) {
        window.shangkeBridge.showToast("未找到课表，请确认已进入「学生课表」页面且课表已加载");
        return;
    }
    ...
    if (courses.length === 0) {
        window.shangkeBridge.showToast("该学期暂无课程数据");
        return;
    }
    ...
    // 步骤 6：保存时间段
    try {
        await window.shangkeBridgePromise.savePresetTimeSlots(
    ...
    // 步骤 7：保存课程
    try {
        await window.shangkeBridgePromise.saveImportedCourses(
    ...
    // 步骤 8：完成
    window.shangkeBridge.showToast("成功导入 " + courses.length + " 门课程（" + semesterName + "）");
