...
async function fetchAndParseCourses(academicYear, semesterCode) {
    ...
    try {
        ...
        window.shangkeBridge.showToast("获取用户信息失败，请确认是否登录教务系统！");
...
async function saveCourses(parsedCourses) {
    ...
    try {
        ...
        window.shangkeBridge.showToast(`成功导入 ${parsedCourses.length} 个规范后的课程块！`);
        return true;
    } catch (error) {
...
async function runImportFlow() {
    window.shangkeBridge.showToast("武汉理工大学课程导入流程启动...");
