...
async function runImportFlow() {
    try {
        if (typeof window.shangkeBridge !== 'undefined') {
            window.shangkeBridge.showToast("正在通过数据接口获取课表...");
        } else {
        ...
        const saveResult = await window.shangkeBridgePromise.saveImportedCourses(JSON.stringify(parsedCourses));
        if (!saveResult) {
            window.shangkeBridge.showToast("保存课程失败，请重试！");
            return;
        }

        window.shangkeBridge.showToast(`成功导入 ${parsedCourses.length} 节课程！`);
