...
async function detectParameters() {
    const html = await request("https://jw.hfnu.edu.cn/eams/courseTableForStd.action?sf_request_type=ajax");
...
async function getSelectedSemester(tagId) {
    const raw = await request(`https://jw.hfnu.edu.cn/eams/dataQuery.action?sf_request_type=ajax`, {
        method: "POST",
    ...
    const idx = await window.shangkeBridgePromise.showSingleSelection("选择学期", JSON.stringify(list.map(s => s.name)), -1);
    return idx !== null ? list[idx] : null;
}

async function fetchAndParseCourses(semesterId, ids) {
    const html = await request(`https://jw.hfnu.edu.cn/eams/courseTableForStd!courseTable.action?sf_request_type=ajax`, {
        method: "POST",
        headers: { "Content-Type": "application/x-www-form-urlencoded" },
...
async function applyTimeSlots() {
    ...
    const timeSlot1 = [
        ...
        { "number": 8, "startTime": "17:00", "endTime": "17:45" },
    ...
    await window.shangkeBridgePromise.savePresetTimeSlots(JSON.stringify(target));
    return true;
}
...
async function runImportFlow() {
    try {
        window.shangkeBridge.showToast("开始探测教务参数...");
        ...
        window.shangkeBridge.showToast("正在同步课表...");
        ...
        if (saveResult) {
            window.shangkeBridge.showToast(`成功导入 ${courses.length} 个课程条目`);
            window.shangkeBridge.notifyTaskCompletion();
        }
    } catch (e) {
        console.error(`[异常] ${e.message}`);
        window.shangkeBridge.showToast(e.message);
    }
}
