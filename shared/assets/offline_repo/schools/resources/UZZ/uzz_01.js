...
async function runImportFlow() {
    ...
    window.shangkeBridge.showToast("正在获取当前页面课表数据...");
    ...
    try {
        ...
        if (courses.length === 0) {
            await window.shangkeBridgePromise.showAlert("导入失败", "该学年学期未找到课程数据，请确认页面上显示的课表是否为空。", "确定");
            return;
        }
        ...
        await window.shangkeBridgePromise.showAlert("导入失败", "接口请求异常，请确认教务系统网络通畅。", "确定");
        console.error(e);
    }
}
