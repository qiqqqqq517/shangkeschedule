...
async function trySaveCalendarInfo(semesterId) {
    try {
        ...
        const saveResult = await window.shangkeBridgePromise.saveCourseConfig(JSON.stringify({
...
async function trySaveTimeSlots() {
    try {
        const saveResult = await window.shangkeBridgePromise.savePresetTimeSlots(JSON.stringify(getZuaTimeSlots()));
        return saveResult === true;
    } catch (error) {
...
async function runImportFlow() {
    try {
        window.shangkeBridge.showToast("开始探测郑航教务参数...");
        ...
        window.shangkeBridge.showToast(buildCompletionMessage(calendarSaved, timeSlotsSaved));
