...
async function saveAppTimeSlots() {
    ...
    return await window.shangkeBridgePromise.savePresetTimeSlots(JSON.stringify(timeSlots));
}
...
async function runImportFlow() {
    try {
        ...
        const year = await window.shangkeBridgePromise.showPrompt("选择学年", "请输入要导入课程的起始学年（例如 2025-2026 应输入2025）:", String(currentYear), "validateYearInput");
        if (!year) return;

        const semesterIndex = await window.shangkeBridgePromise.showSingleSelection("选择学期", JSON.stringify(["第一学期", "第二学期"]), 0);
        ...
        await window.shangkeBridgePromise.saveImportedCourses(JSON.stringify(finalCourses)); //[cite: 1]
